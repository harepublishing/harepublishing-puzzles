...
window.HareKnightsKnavesPlatformEngine = {
  init({
    ...
    function getCardResult(letter, state = states[currentMode], cfg = getConfig()) {
      ...
      if (state.evaluated && picked) return picked === correct ? "correct" : "incorrect";
      return "";
    }
    ...
    function renderAnswerControls() {
      ...
      return `<div class="hp-knights-answer-grid" role="group" aria-label="Choose knight or knave for each person">
        ${cfg.letters.map(letter => {
          ...
          return `<div class="hp-knights-person-card ${result ? `is-${result}` : ""}" data-person="${letter}">
            ...
            <div class="hp-knights-choice-row">
              <button type="button" data-a="answer" data-person="${letter}" data-value="K" class="hp-knights-choice ${answerForButton === "K" ? "active" : ""}" ${locked ? "disabled" : ""}>Knight</button>
              <button type="button" data-a="answer" data-person="${letter}" data-value="N" class="hp-knights-choice ${answerForButton === "N" ? "active" : ""}" ${locked ? "disabled" : ""}>Knave</button>
            </div>
    ...
    function renderUI() {
      ...
      mount.innerHTML = `<style>
        ...
        #hp-knights-container .hp-knights-top{display:flex;align-items:center;justify-content:flex-start;gap:12px;margin:0 0 12px;padding:8px 10px;border:1px solid #edf2f6;border-radius:16px;background:#fff;}
        ...
        #hp-knights-container .hp-knights-completion-status.show{display:block;}
                #hp-knights-container .hp-knights-statements{background:#fff;border:1px solid #e9eef3;border-radius:18px;padding:13px 14px;margin:0 0 12px;box-shadow:0 8px 24px rgba(0,0,0,.045);}
        #hp-knights-container .hp-knights-statements h3{margin:0 0 8px;color:var(--hp-primary-dark);font-size:18px;}
        #hp-knights-container blockquote{margin:8px 0;padding:9px 11px;border-left:5px solid var(--hp-primary);border-radius:12px;background:#fafafa;color:#222;font-size:15px;line-height:1.34;}
        #hp-knights-container .hp-knights-logic{margin-top:9px;background:var(--hp-primary-light);border:1px solid var(--hp-primary-soft);border-radius:14px;padding:8px 10px;}
        #hp-knights-container .hp-knights-logic summary{cursor:pointer;font-weight:900;color:var(--hp-primary-dark);}
        #hp-knights-container .hp-knights-logic pre{white-space:pre-wrap;font-size:16px;line-height:1.35;margin:8px 0 0;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;}
        ...
        #hp-knights-container .hp-knights-person-card{position:relative;border:1px solid #e9eef3;border-radius:18px;background:#fff;padding:12px;box-shadow:0 8px 22px rgba(0,0,0,.045);transition:background-color .18s ease,border-color .18s ease;}
        ...
        #hp-knights-container .hp-knights-card-icon{position:absolute;top:9px;left:9px;font-size:22px!important;line-height:1!important;font-variation-settings:'FILL' 1,'wght' 700,'GRAD' 0,'opsz' 40!important;}
        ...
        #hp-knights-container .hp-knights-person-id{display:flex;align-items:center;gap:8px;justify-content:center;margin-bottom:8px;padding-left:18px;}
        #hp-knights-container .hp-knights-person-id span{display:inline-flex;align-items:center;justify-content:center;width:30px;height:30px;border-radius:50%;background:var(--hp-primary);color:#fff;font-weight:900;}
        #hp-knights-container .hp-knights-person-id strong{font-size:16px;color:#111;}
        #hp-knights-container .hp-knights-choice-row{display:grid;grid-template-columns:1fr 1fr;gap:8px;}
        #hp-knights-container .hp-knights-choice{border:2px solid var(--hp-primary-soft);background:#fff;color:var(--hp-primary-dark);border-radius:13px;padding:8px 7px;font-weight:900;cursor:pointer;font-family:inherit;}
