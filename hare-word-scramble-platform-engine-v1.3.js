...
window.HareWordScramblePlatformEngine = (() => {
  const VERSION = "word-scramble-platform-engine-v1.3";
  ...
  const CSS = `
    ...
    @media(max-width:560px){
      ...
      #hp-wordscramble-container .hpwsc-info-buttons { grid-template-columns:1fr; }
      #hp-wordscramble-container .hpwsc-info-body { min-height:76px; }
      #hp-wordscramble-container .hpwsc-slot,
  ...
  function injectStyles(){
    if (document.getElementById("hp-word-scramble-platform-engine-css-v13")) return;
    const style = document.createElement("style");
    style.id = "hp-word-scramble-platform-engine-css-v13";
  ...
  function renderInfoPanel(entry, solved, revealed, disabled){
    ...
    if (solved) {
      ...
      bodyHtml=`<strong>✓ Correct!</strong><br>You solved <strong>${escapeHtml(entry.answer)}</strong>. Choose another word when you're ready.`;
    } else if (revealed) {
      bodyClass="is-revealed";
      bodyHtml=`<strong><span class="material-symbols-outlined" aria-hidden="true">visibility</span> Revealed</strong><br>The word was <strong>${escapeHtml(entry.answer)}</strong>. Choose another word when you're ready.`;
