/* =========================================================
   ...
   Updated: 2026-08-19 v3.2 - sends each analytics event through one route only

   Suggested filename:
   hare-word-search-platform-engine-v3.2.js
...
window.HareWordSearchEngine = {
  init({
    ...
    function renderWordLabel(display, word) {
      const label = String(display || "");
      const tokenPattern = /[\p{L}\p{M}]+(?:[’'.-][\p{L}\p{M}]+)*/gu;
      const uppercaseRanges = [];
      let match;

      while ((match = tokenPattern.exec(label))) {
        const letters = match[0].replace(/[^\p{L}\p{M}]/gu, "");
        const hasCasedLetter = letters.toLocaleUpperCase() !== letters.toLocaleLowerCase();
        const gridLetters = cleanDisplayWord(letters);
        if (gridLetters.length >= 2 && hasCasedLetter && letters === letters.toLocaleUpperCase()) {
          uppercaseRanges.push({ start: match.index, end: match.index + match[0].length, text: match[0] });
        }
      }

      const uppercaseWord = cleanDisplayWord(uppercaseRanges.map(range => range.text).join(""));
      const targetRanges = uppercaseWord === word && uppercaseRanges.length
        ? uppercaseRanges
        : [{ start: 0, end: label.length, text: label }];
      const parts = [];
      let cursor = 0;

      targetRanges.forEach(range => {
        if (range.start > cursor) {
          parts.push(`<span class="hp-ws-word-context">${escapeHtml(label.slice(cursor, range.start))}</span>`);
        }
        parts.push(`<span class="hp-ws-word-target">${escapeHtml(label.slice(range.start, range.end))}</span>`);
        cursor = range.end;
      });

      if (cursor < label.length) {
        parts.push(`<span class="hp-ws-word-context">${escapeHtml(label.slice(cursor))}</span>`);
      }
    ...
    function displayKeys(value) {
      ...
      const keys = [];
      if (label.includes(",")) {
        const surname = cleanDisplayWord(label.split(",")[0]);
        if (surname) keys.push(surname);
      }
      const complete = cleanDisplayWord(label);
      if (complete && !keys.includes(complete)) keys.push(complete);
      return keys;
    }
    ...
    const displayWords = Array.isArray(pageData.words)
      ? pageData.words.map(value => String(value || "").trim()).filter(Boolean)
      : [];
    ...
    const displayRecords = displayWords.map((display, index) => ({
      display,
      index,
      keys: displayKeys(display)
    }));
    const usedDisplayIndexes = new Set();
    function displayForPlacement(rawPlacement, word, placementIndex) {
      const explicit = String(rawPlacement.display || "").trim();
      if (explicit) return explicit;
      const semantic = displayRecords.find(record => !usedDisplayIndexes.has(record.index) && record.keys.includes(word));
      if (semantic) {
        usedDisplayIndexes.add(semantic.index);
        return semantic.display;
      }
      if (displayRecords.length === placementsRaw.length && displayRecords[placementIndex] && !usedDisplayIndexes.has(placementIndex)) {
        usedDisplayIndexes.add(placementIndex);
        return displayRecords[placementIndex].display;
      }
      return word;
    }
    const placements = placementsRaw.map((p, placementIndex) => {
      const word = cleanWord(p.word);
      const display = displayForPlacement(p, word, placementIndex);
      const row = Number(p.row);
      const col = Number(p.col);
      const dr = Number(p.dr);
      const dc = Number(p.dc);
      const path = getPathForPlacement({ word, row, col, dr, dc });
      return { word, display, row, col, dr, dc, path, pathKey: canonicalPathKey(path) };
    ...
    const wordEntries = placements.map(p => ({ word: p.word, display: p.display }));
    ...
    function loadState() {
      try {
        ...
        const merged = raw ? { ...defaultState(), ...JSON.parse(raw) } : defaultState();
        if (merged.solved || merged.revealed) {
          // Current JSON placements, not a cached path list, define the answer.
          merged.foundWords = [...normalizedWords];
          merged.foundPathKeys = placements.map(placement => placement.pathKey);
          merged.anchor = null;
        }
        return merged;
      } catch {
    ...
    function getPersistableState() {
      ...
      return {
        solved:Boolean(state.solved), revealed:Boolean(state.revealed),
        startedAt:state.startedAt || "", solvedAt:state.solvedAt || "",
        revealedAt:state.revealedAt || "", overlaySeen:Boolean(state.overlaySeen)
      };
    }
    ...
    function saveState() {
      try {
        const serialized = JSON.stringify(getPersistableState());
        localStorage.setItem(STORAGE_KEY, serialized);
        if (state.solved || state.revealed) localStorage.setItem(STORAGE_KEY, serialized);
    ...
    if (state.solved || state.revealed) saveState();
    ...
    function resetPuzzle() {
      ...
      const progressCore = window.HarePuzzleCore;
      if (progressCore && typeof progressCore.removeSavedState === "function") progressCore.removeSavedState(STORAGE_KEY, { allowDowngrade:true });
      else localStorage.removeItem(STORAGE_KEY);
      localStorage.removeItem(`hp_backup__${STORAGE_KEY}`);
    ...
    function renderWordListOnly() {
      ...
      listEl.innerHTML = wordEntries
        .slice()
        .sort((a, b) => a.display.localeCompare(b.display, undefined, { sensitivity: "base" }) || a.word.localeCompare(b.word))
        .map(entry => {
          ...
          return `<button type="button" class="${classes.join(" ")}" data-word="${escapeHtml(word)}"${disabled}>${icon}<span class="hp-ws-word-text">${renderWordLabel(display, word)}</span></button>`;
        })
        .join("");
