window.HareKrissKrossEngine = {
  ...
  init({ containerId = "hp-krisskross-container", dataObject } = {}) {
    ...
    function injectStyles() {
      if (document.getElementById("hp-kriss-kross-platform-engine-css-v10")) return;
      const style = document.createElement("style");
      style.id = "hp-kriss-kross-platform-engine-css-v10";
      style.textContent = CSS + `
/* =========================================================
   PLATFORM LAYOUT REFINEMENTS v1.3
   Use production Kriss Kross board model inside platform shell.
   ========================================================= */
#hp-krisskross-container .hp-kk-layout{display:grid !important;grid-template-columns:minmax(0,1fr) 225px !important;gap:18px !important;align-items:stretch !important;width:100% !important;max-width:100% !important;overflow:visible !important;}
...
#hp-krisskross-container .hp-kk-col-right{width:225px !important;min-width:225px !important;max-width:225px !important;}
#hp-krisskross-container .hp-kk-col-left .hp-kk-panel,#hp-krisskross-container .hp-kk-col-right .hp-kk-panel{height:720px !important;min-height:720px !important;display:flex !important;flex-direction:column !important;}
#hp-krisskross-container .hp-kk-theme{margin:0 0 12px !important;color:#2f4667 !important;font-size:24px !important;line-height:1.15 !important;font-weight:900 !important;text-align:center !important;}
#hp-krisskross-container .hp-kk-status,#hp-krisskross-container .hp-kk-stats,#hp-krisskross-container .hp-kk-progress{display:none !important;}
...
#hp-krisskross-container .hp-kk-board{background:transparent !important;border:0 !important;padding:0 !important;margin:0 auto !important;}
#hp-krisskross-container .hp-kk-block{background:transparent !important;border:0 !important;visibility:hidden !important;pointer-events:none !important;}
...
#hp-krisskross-container .hp-puzzle-tools .clear-tool{grid-column:auto !important;}
#hp-krisskross-container .hp-puzzle-mobile-tools{display:none !important;}
#hp-krisskross-container .hp-kk-words-header{display:block !important;margin:0 0 10px !important;}
#hp-krisskross-container .hp-kk-words-header h3{margin:0 0 4px !important;color:#2f4667 !important;font-size:18px !important;line-height:1.15 !important;font-weight:900 !important;text-align:center !important;}
#hp-krisskross-container .hp-kk-pill{display:block !important;padding:0 !important;border:0 !important;border-radius:0 !important;background:transparent !important;color:#3d4b58 !important;font-size:13px !important;line-height:1.25 !important;font-weight:900 !important;text-align:center !important;white-space:normal !important;}
...
#hp-krisskross-container .hp-kk-board{
  ...
  gap:var(--hp-kk-gap,3px) !important;
...
#hp-krisskross-container .hp-kk-cell,
#hp-krisskross-container .hp-kk-block{
  width:var(--hp-kk-cell-size,29px) !important;
  height:var(--hp-kk-cell-size,29px) !important;
  min-width:var(--hp-kk-cell-size,29px) !important;
  min-height:var(--hp-kk-cell-size,29px) !important;
}
#hp-krisskross-container .hp-kk-block{
  background:transparent !important;
  border:0 !important;
...
#hp-krisskross-container .hp-kk-cell.is-selected{
  ...
  color:#16324a !important;
  border-color:#7ea9d1 !important;
  box-shadow:inset 0 0 0 1px #6e8fb9 !important;
}
#hp-krisskross-container .hp-kk-cell.is-selected.is-slot-start{
  background:#107FBB !important;
  color:#fff !important;
  border-color:#0d699a !important;
  box-shadow:inset 0 0 0 1px #0d699a !important;
}
@media(max-width:760px){#hp-krisskross-container .hp-kk-board-wrap{padding:12px 6px 10px !important;}}
`;
    ...
    function injectSchema() {
      ...
      const datePublished = String(data.puzzleDate || "").trim();

      const schemaData = {
        "@context": "https://schema.org",
        "@type": "Game",
        "name": puzzleTitle,
        "description": `Play ${puzzleTitle} online from Hare Publishing. Fit all of the words into the crossing grid.`,
        "url": pageUrl,
        "genre": "Puzzle",
        "inLanguage": "en",
        "publisher": {
      ...
      if (datePublished) schemaData.datePublished = datePublished;
    ...
    window.HareKrissKrossEngine.statusAdapter = {
      isSolved(saved) {
        return Boolean(saved && !this.isRevealed(saved) && (saved.solved || saved.completed || saved.isSolved || saved.status === "solved" || saved.status === "complete" || saved.completedAt || saved.solvedAt));
      },
      isRevealed(saved) {
        return Boolean(saved && (saved.revealed || saved.revealedAt || saved.status === "revealed"));
      },
      isFinished(saved) {
        return Boolean(saved && (this.isSolved(saved) || this.isRevealed(saved) || saved.finishedAt));
      },
      hasProgress(saved) {
        if (!saved || this.isFinished(saved)) return false;
        return Boolean((saved.assignments && Object.keys(saved.assignments).length > 0) || saved.selectedSlotId || saved.startedAt || saved.updatedAt || saved.lastPlayedAt);
      },
      finishedDate(saved) {
    ...
    function emitKrissKrossStateChange(action = "save") {
      try {
        const detail = {
          ...
          status: state.solved ? "solved" : state.revealed ? "revealed" : Object.keys(state.assignments || {}).length ? "in-progress" : "not-started",
          action
        };
    ...
    function saveState(action = "save") {
      try {
        ...
        emitKrissKrossStateChange(action);
      } catch {}
    }
    ...
    function adjustBoardSize() {
      ...
      if (!boardEl || !rowCount || !colCount) return;

      // Match the current production Kriss Kross board model.
      // The production puzzle uses a compact crossword-style grid with fixed cells,
      // transparent blocks, and rounded active squares. Do not stretch the grid into
      // the available panel space.
      const isSmall = window.matchMedia("(max-width: 560px)").matches;
      const isTablet = window.matchMedia("(max-width: 760px)").matches;
      const cellSize = isSmall ? 22 : (isTablet ? 24 : 29);
      const gap = isSmall ? 2 : 3;

      boardEl.style.setProperty("--hp-kk-gap", `${gap}px`);
      boardEl.style.setProperty("--hp-kk-cell-size", `${cellSize}px`);
    ...
    function renderBoardOnly() {
      ...
      adjustBoardSize();
      window.requestAnimationFrame(adjustBoardSize);
    }
    ...
    function getPlatformStats() {
      ...
      const adapter = window.HareKrissKrossEngine.statusAdapter;
      if (Core && typeof Core.getStats === "function" && adapter) {
        return Core.getStats({ storagePrefix: "hp_kk_", statusAdapter: adapter });
      }
      return { streak: 0, solved: 0, revealed: 0, inProgress: 0, played: 0 };
    }
    ...
    function renderRecommendationHtml() {
        ...
        if (nextPuzzle && nextPuzzle.puzzleId) {
          return `
            <div class="hp-recommend-card">
              ...
              <div class="hp-recommend-copy">Your next available Kriss Kross puzzle is ready. Keep the fun going with another crossing-word challenge.</div>
      ...
      return `
        <div class="hp-recommend-card">
          ...
          <div class="hp-recommend-copy">Congratulations — every available Kriss Kross puzzle has been played.</div>
      ...
      mount.querySelectorAll("[data-a]").forEach(btn => {
        btn.addEventListener("click", () => {
          ...
          if (action === "load-puzzle") {
            ...
            hideOverlay();
            if (nextId && typeof window.HareKrissKrossLoadPuzzle === "function") window.HareKrissKrossLoadPuzzle(nextId, { scroll: false });
            else if (nextId) window.location.href = `${window.location.pathname || "/kriss-kross"}?puzzle=${encodeURIComponent(nextId)}`;
            return;
          }
    ...
    window.removeEventListener("resize", container.__hpKrissKrossResizeHandler || (() => {}));
    container.__hpKrissKrossResizeHandler = () => adjustBoardSize();
    window.addEventListener("resize", container.__hpKrissKrossResizeHandler);
  ...
  openHelp(containerId = "hp-krisskross-container") {
    ...
    let modal = target?.querySelector?.("#hp-kk-help-modal") || mount?.querySelector?.("#hp-kk-help-modal");

    if (!modal && typeof this.init === "function") {
      try { render(); } catch {}
      modal = target?.querySelector?.("#hp-kk-help-modal") || mount?.querySelector?.("#hp-kk-help-modal");
    }
    ...
    alert("Select a slot in the grid, then choose a matching word from the list. If a square belongs to two words, click it again to switch direction. Hint mode highlights words that match the selected slot length.");
  }
};