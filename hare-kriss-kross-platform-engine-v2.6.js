window.HareKrissKrossEngine = {
  init({ containerId = "hp-krisskross-container", dataObject } = {}) {
    ...
    function renderOverlayContent() {
      ...
      badgeMetaEl.innerHTML = `<span class="hp-result-stat-chip"><span class="material-symbols-outlined" aria-hidden="true">check_circle</span><strong>${correctCount()}</strong> of ${normalizedPlacements.length} words placed</span>`;

      if (state.solved) {
        overlayIconEl.textContent = "celebration";
        overlayTitleEl.textContent = "You Solved the Kriss Kross!";
        overlayTextEl.innerHTML = `
          <div class="hp-modal-lead">Congratulations — you placed every word!</div>
          <div class="hp-modal-subtext">Great job solving this Kriss Kross puzzle.</div>
          <div class="hp-recommend-card">
            <div class="hp-recommend-title">Keep your streak going</div>
            <div class="hp-recommend-copy">Try another puzzle from the Puzzlers Hub or explore puzzle books for offline play.</div>
          </div>
        `;
        return;
      }
      ...
      if (state.revealed) {
        ...
        overlayTextEl.innerHTML = `
          <div class="hp-modal-lead">Completed with a little help —<br>the answers were revealed.</div>
          <div class="hp-recommend-card">
            <div class="hp-recommend-title">Try another Kriss Kross</div>
            <div class="hp-recommend-copy">Choose another puzzle from the Puzzlers Hub or keep this one open to study the completed grid.</div>
          </div>
        `;
      }
    }
