    ...
    function getOverlayNextActionsHtml() {
      if (typeof window.HareKnightsGetNextPuzzleOptions !== "function") {
        return typeof window.HareKnightsRequestNextPuzzle === "function"
          ...
          : `<a class="hp-knights-primary" href="/online-puzzles">More Puzzles</a>`;
      }
      ...
      function addAction(kind, item, prefix) {
        if (!item || !item.puzzleId || seen.has(String(item.puzzleId))) return;
        seen.add(String(item.puzzleId));
        actions.push(`<button type="button" class="hp-knights-primary" data-a="request-next" data-next-kind="${escapeHtml(kind)}">${escapeHtml(prefix)} ${escapeHtml(`Knights & Knaves #${item.puzzleId}`)}</button>`);
      }
