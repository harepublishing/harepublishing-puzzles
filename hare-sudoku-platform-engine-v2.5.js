        ...
        overlayNextPanelEl.innerHTML = `
          ...
          <div class="hp-sudoku-next-mode-buttons">
            ${nextByMode.map(item => {
              ...
              return `<button class="hp-sudoku-next-panel-btn" type="button" data-a="overlay-next-puzzle" data-puzzle-id="${escapeHtml(item.puzzleId)}" data-mode="${escapeHtml(item.mode)}">Next ${escapeHtml(modeLabel)}</button>`;
            }).join("")}
          </div>
        `;
