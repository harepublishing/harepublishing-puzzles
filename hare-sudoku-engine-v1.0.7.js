...
window.HareSudokuEngine = {
  init({
    ...
    if (!container) {
      console.error("HareSudokuEngine: puzzle container missing.");
      return;
    }
    ...
    if (container.dataset.hpSudokuMounted === "true") {
      console.warn("HareSudokuEngine: this container has already been mounted.");
      return;
    }
    ...
    if (!mount) {
      console.error("HareSudokuEngine: .hp-mount element missing inside puzzle container.");
      return;
    }
    ...
    if (!pageData && dataEl) {
      try {
        ...
        console.error("HareSudokuEngine: puzzle data block contains invalid JSON.", err);
      }
    }
    ...
    function saveState(mode, state) {
      try {
        ...
        console.warn("HareSudokuEngine: unable to save puzzle progress.", err);
      }
    }
    ...
    function getPuzzleDisplayName(cfg = getConfig()) {
      ...
      const baseName = cfg?.mode === "challenge" ? "Daily Sudoku Challenge" : (cfg?.label || "Sudoku");
