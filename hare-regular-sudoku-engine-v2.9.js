...
window.HareRegularSudokuEngine = {
  init({
    ...
    const SUDOKU_MODE_ORDER = ["daily", "easy", "medium", "hard", "extreme"];

    const MODE_META = {
      daily: {
        mode: "daily",
        label: "Daily Sudoku",
        boardAria: "Daily Sudoku Board",
        schemaName: () => `Daily Sudoku Puzzle`,
        schemaDescription: "Play and solve today's Daily Sudoku Challenge by Hare Publishing. Includes notes, timer, and progress saving.",
        solvedTitle: "You Solved the Daily Sudoku Puzzle!",
        solvedStat: "Daily Sudoku Solved! 🎉 ",
        ...
        revealedText: "Here is the completed Daily Sudoku puzzle. New puzzles are added daily in the Puzzlers Hub, or you can explore a whole stack of puzzles to enjoy offline.",
        footer: "Hare Publishing • Daily Sudoku",
        shareTitle: () => `Daily Sudoku — Hare Publishing`,
        shareSolvedText: (id, time) => `I solved today’s Daily Sudoku in ${time}!`,
        shareRevealedText: () => `I revealed the answer for Daily Sudoku at Hare Publishing.`,
        sharePlayingText: () => `I’m playing today’s Daily Sudoku!`,
    ...
    function buildPuzzlesFromData(data) {
      ...
      // Supports the regular multi-level format:
      ...
      // Supports a single Daily Sudoku format:
      // { puzzleId: "...", puzzle: "...", solution: "..." }
      // This creates one playable puzzle and hides the difficulty tabs.
      if (!Object.keys(result).length && source.puzzle && source.solution) {
        const puzzleId = String(source.puzzleId || source.id || "").trim();
    ...
    if (!availableModes.length) {
      showConfigError("At least one Sudoku puzzle is required. Add a single puzzle, or add easy, medium, hard, or extreme puzzle data.");
      return;
    }

    const showLevelChooser = availableModes.length > 1;
    ...
    let cells = [];
    const TOOLS_OPEN_KEY = "hp_sudoku_tools_open_v1";
    let toolsOpen = (() => {
      try {
    ...
    function applyMobileKeypadSizing() {
      ...
      mount.querySelectorAll(".hp-btn-grid button[data-n]").forEach(btn => {
        if (shouldShorten) {
...
<style>
  ...
  #hp-sudoku-container .hp-rs-top-area.single-puzzle {
  ...
  @media (max-width: 480px) {
    ...
    #hp-sudoku-container .hp-rs-level-panel .hp-mode-btn {
     ...
     <div class="hp-puzzle-date">
        ...
        <div class="hp-rs-top-area ${showLevelChooser ? "" : "single-puzzle"}">
          ${showLevelChooser ? `
    ...
    function bindUIEvents() {
      ...
      mountClickHandler = (e) => {
        ...
        if (a === "tools-toggle") {
          toolsOpen = !toolsOpen;
          try {
            localStorage.setItem(TOOLS_OPEN_KEY, toolsOpen ? "true" : "false");
          } catch {}
          updateToolsTray();
          return;
        }
