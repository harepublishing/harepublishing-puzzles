...
window.HareKrissKrossEngine = {
  init({ containerId = "hp-krisskross-container", dataObject } = {}) {
    ...
    (() => {
      ...
      const schemaDate = String(data.puzzleDate || data.date || "").trim();
      const collectionUrl = new URL(data.publicPlayUrl || data.playPageUrl || "/kriss-kross", window.location.origin).toString();
      const schemaData = {
        ...
        "keywords": ["Kriss Kross", "word puzzle", "crossword-style puzzle", "daily puzzle", "online puzzle", "Hare Publishing"],
        "publisher": {
          "@type": "Organization",
    ...
    function saveState() {
      try {
        ...
        window.dispatchEvent(new CustomEvent("hare:puzzle-state-change", { detail: { puzzleType: "kriss-kross", puzzleId } }));
        window.dispatchEvent(new CustomEvent("hare-kriss-kross-progress", { detail: { puzzleId } }));
      } catch {}
    }
    ...
    function renderRecommendationHtml(context = {}){
      if (typeof window.HareKrissKrossFindNextPuzzle === "function") {
        ...
        if (nextPuzzle && nextPuzzle.puzzleId) {
          const nextId = String(nextPuzzle.puzzleId);
          const isContinue = Boolean(nextPuzzle.status === "in-progress" || nextPuzzle.isInProgress);
          const messageTitle = context.solved
            ? "Congratulations — you placed every word!"
            : (isContinue ? "Continue where you left off" : "Another Kriss Kross is ready");
          const messageCopy = isContinue
            ? "Pick up where you left off and finish placing the words."
            : "Keep your puzzle streak going with the next available Kriss Kross.";
          const actionLabel = isContinue ? "Continue" : "Play";

          return `
            <div class="hp-kk-overlay-next-copy">
              <strong>${escapeHtml(messageTitle)}</strong>
              <span>${escapeHtml(messageCopy)}</span>
            </div>
            <div class="hp-kk-modal-buttons">
              <button class="hp-link-btn primary" type="button" data-a="load-puzzle" data-puzzle-id="${escapeHtml(nextId)}">
                ${actionLabel} Kriss Kross #${escapeHtml(nextId)}
              </button>
    ...
    function renderOverlayContent() {
      ...
      const overlayStatusEl = mount.querySelector("#hp-kk-overlay-status");
    ...
    function bindDynamicOverlayActions() {
      mount.querySelectorAll('[data-a="load-puzzle"]').forEach(btn => {
        ...
        btn.dataset.hpKkDynamicBound = "1";
        btn.addEventListener("click", () => {
      ...
      mount.innerHTML = `
        ...
        <div class="hp-kk-layout">
          <div class="hp-kk-col-left">
            <div class="hp-kk-panel">
              ...
              ${puzzleTheme ? `
                <div class="hp-kk-theme-title">${escapeHtml(puzzleTheme)}</div>
              ` : ""}
        ...
        <div class="hp-overlay" id="hp-kk-overlay" aria-hidden="true">
          <div class="hp-modal" role="dialog" aria-modal="true" aria-label="Kriss Kross result">
            <button type="button" class="hp-kk-overlay-close" data-a="close-overlay" aria-label="Close result card">
              <span class="material-symbols-outlined" aria-hidden="true">close</span>
            </button>

            <span id="hp-kk-overlay-icon" class="material-symbols-outlined hp-kk-overlay-icon" aria-hidden="true">celebration</span>
            <div id="hp-kk-overlay-status" class="hp-kk-overlay-status">Solved</div>
            <h3 id="hp-kk-overlay-title">Kriss Kross Puzzle</h3>
...
// Kriss Kross platform engine v4.2.
// Clean production-ready build for the platform page.
// v4.2: Success/Reveal overlay now matches the approved Hare Publishing result card standard.
