...
window.HareWordScramblePlatformEngine = (() => {
  const VERSION = "word-scramble-platform-engine-v2.7";
  ...
  function renderRecommendationHtml(){
    ...
    if(nextPuzzle?.puzzleId){
      ...
      const title = isProgress ? "Continue where you left off" : "Another Word Scramble is ready";
      ...
      const buttonLabel = isProgress ? `Continue Word Scramble #${nextPuzzle.puzzleId}` : `Word Scramble #${nextPuzzle.puzzleId}`;

      return `<div class="hp-recommend-card">
        <div class="hp-recommend-title">${escapeHtml(title)}</div>
