/* =========================================================
   ...
   Updated: 2026-07-27 v3.0 - uses top-level words as display labels while placements remain normalized

   Suggested filename:
   hare-word-search-platform-engine-v3.0.js
    ...
    function renderWordListOnly() {
      ...
      listEl.innerHTML = wordEntries
        ...
        .map(entry => {
          ...
          return `<button type="button" class="${classes.join(" ")}" data-word="${escapeHtml(word)}"${disabled}>${icon}<span class="hp-ws-word-text">${escapeHtml(display)}</span></button>`;
        })
        .join("");
