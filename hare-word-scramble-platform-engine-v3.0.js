...
window.HareWordScramblePlatformEngine = (() => {
  const VERSION = "word-scramble-platform-engine-v2.9";
  ...
  const CSS = `
    ...
    #hp-wordscramble-container #hp-wsc-overlay .hp-overlay-close {
      position:absolute;
      top:10px;
      right:10px;
      width:34px;
    ...
    #hp-wordscramble-container #hp-wsc-overlay .hp-overlay-close:hover,
    #hp-wordscramble-container #hp-wsc-overlay .hp-overlay-close:focus-visible {
      background:#25A28D;
      color:#fff;
      outline:none;
    }

    #hp-wordscramble-container #hp-wsc-overlay .hp-overlay-close .material-symbols-outlined {
    ...
    #hp-wordscramble-container .hp-overlay-status {
      margin:0 0 8px;
      font-size:24px;
      line-height:1.1;
      font-weight:900;
    ...
    #hp-wordscramble-container .hp-result-meta { margin:12px 0 14px; text-align:center; }
    #hp-wordscramble-container .hp-result-puzzle-title { display:none; }
    #hp-wordscramble-container .hp-result-stats-line { display:flex; flex-wrap:nowrap; justify-content:center; align-items:center; gap:0 18px; color:#555; font-size:12px; line-height:1.25; font-weight:800; white-space:nowrap; margin:0 auto 14px; }
    #hp-wordscramble-container .hp-result-stat-chip { display:inline-flex; align-items:center; gap:4px; white-space:nowrap; }
    #hp-wordscramble-container .hp-result-stat-chip .material-symbols-outlined { font-size:16px; color:#167966; font-variation-settings:'FILL' 1,'wght' 700,'GRAD' 0,'opsz' 40; }
    ...
    #hp-wordscramble-container .hp-recommend-title { font-size:22px; font-weight:900; color:#555; margin:0 0 4px; line-height:1.25; text-align:center; }
    #hp-wordscramble-container .hp-recommend-copy { font-size:18px; color:#555; line-height:1.25; margin:0 auto 4px; font-weight:800; text-align:center; }
    #hp-wordscramble-container .hp-modal-lead { font-size:22px; line-height:1.25; font-weight:900; margin:18px auto 4px; color:#555; text-align:center; }
    #hp-wordscramble-container .hp-modal-subtext { font-size:18px; color:#555; line-height:1.25; margin:0 auto 4px; font-weight:800; text-align:center; }
    ...
    #hp-wordscramble-container #hp-wsc-overlay .hp-link-btn {
      min-height:42px;
      min-width:220px;
      padding:10px 22px;
      font-size:12px;
    ...
    #hp-wordscramble-container .hp-link-btn.share { background:#EAF8F5; color:#167966; border-color:#BFE7DF; box-shadow:0 4px 12px rgba(37,162,141,.12); }
    ...
    @media(max-width:560px){
      ...
      #hp-wordscramble-container #hp-wsc-overlay .hp-link-btn { min-width:min(190px,100%); width:100%; }
      #hp-wordscramble-container .hp-link-btn.full { grid-column:auto; }
    }
  ...
  function injectStyles(){
    if (document.getElementById("hp-word-scramble-platform-engine-css-v28")) return;
    const style = document.createElement("style");
    style.id = "hp-word-scramble-platform-engine-css-v28";
  ...
  function injectSchema(){
    ...
    const collectionUrl = new URL(puzzle.publicPlayUrl || puzzle.playPageUrl || "/word-scramble", window.location.origin).toString();
    ...
    const schemaData = {
      ...
      "copyrightYear": String((puzzleDate && puzzleDate.slice(0, 4)) || new Date().getFullYear()),
      "keywords": ["Word Scramble", "word puzzle", "daily puzzle", "online puzzle", "Hare Publishing"],
      "publisher": {
        "@type": "Organization",
  ...
  function renderRecommendationHtml(){
    ...
    if(typeof window.HareWordScrambleFindNextPuzzle === "function"){
      nextPuzzle = window.HareWordScrambleFindNextPuzzle(puzzle.puzzleId);
    }

    if(nextPuzzle?.puzzleId){
      const status = String(nextPuzzle.status || nextPuzzle.puzzleStatus || "").toLowerCase();
      const isProgress = status === "in-progress" || status === "progress";
      const copy = isProgress
        ? "Pick up where you left off and keep building your streak."
        : "Try another Word Scramble puzzle and keep building your streak.";
      const buttonLabel = isProgress ? `Continue Word Scramble #${nextPuzzle.puzzleId}` : `Play Word Scramble #${nextPuzzle.puzzleId}`;

      return `<div class="hp-recommend-card">
        <div class="hp-recommend-copy">${escapeHtml(copy)}</div>
        <div class="hp-modal-actions">
          <button class="hp-link-btn primary" data-a="load-puzzle" data-puzzle-id="${escapeHtml(nextPuzzle.puzzleId)}">${escapeHtml(buttonLabel)}</button>
  ...
  function renderOverlayContent(){
    ...
    const overlayStatusEl=mount.querySelector("#hp-wsc-overlay-status");
    ...
    if(state.solved){
      ...
      overlayTextEl.innerHTML=`<div class="hp-modal-lead">Answers revealed.</div><div class="hp-modal-subtext">Completed with a little help.</div>${renderRecommendationHtml()}`;
    }
  }
  ...
  function renderWordList(){
    return `<div class="hpwsc-panel hpwsc-word-panel"><h3>Word List</h3><span class="hpwsc-word-progress">Words Unscrambled: ${solvedCount().toLocaleString()}/${puzzleData.length.toLocaleString()}</span><div class="hpwsc-word-list">${renderWordItems()}</div></div>`;
  }
  function renderLiveState(){
    const puzzlePanel=mount?.querySelector(".hpwsc-puzzle-panel");
    const progress=mount?.querySelector(".hpwsc-word-progress");
    const list=mount?.querySelector(".hpwsc-word-list");
    if(!puzzlePanel||!progress||!list){render();return;}
    puzzlePanel.innerHTML=`${puzzle.theme?`<h3 class="hpwsc-theme">${escapeHtml(puzzle.theme)}</h3>`:""}${renderCurrent()}`;
    progress.textContent=`Words Unscrambled: ${solvedCount().toLocaleString()}/${puzzleData.length.toLocaleString()}`;
    list.innerHTML=renderWordItems();
  }
  ...
  function render(){
    ...
    mount.innerHTML=`<div class="hpwsc-shell"><div class="hpwsc-card">
      ...
      <div class="hp-overlay" id="hp-wsc-overlay" aria-hidden="true"><div class="hp-modal" role="dialog" aria-modal="true" aria-label="Word Scramble result"><button class="hp-overlay-close" type="button" data-a="close-overlay" aria-label="Close result card"><span class="material-symbols-outlined" aria-hidden="true">close</span></button><span id="hp-wsc-overlay-icon" class="material-symbols-outlined" aria-hidden="true">celebration</span><div id="hp-wsc-overlay-status" class="hp-overlay-status">Solved</div><h3 id="hp-wsc-overlay-title">Word Scramble Puzzle</h3><div class="hp-result-meta"><div class="hp-result-puzzle-title" id="hp-wsc-badge-id"></div><div class="hp-result-stats-line" id="hp-wsc-badge-meta"></div></div><div id="hp-wsc-overlay-text"></div></div></div>
