/* HARE PUBLISHING PUZZLE DATA MANAGER v1.0
...
(function (window, document) {
  ...
  function downloadBackup(setStatus) {
    ...
    link.href = url;
    link.download = `hare-puzzle-progress-${new Date().toISOString().slice(0, 10)}.json`;
  ...
  function init({ containerId = "hp-puzzle-data-manager" } = {}) {
    ...
    host.innerHTML = `<div class="hp-pdm-footer"><div class="hp-pdm-footer-title"><span class="material-symbols-outlined" aria-hidden="true">cloud_sync</span>Protect Your Puzzle Progress</div><p class="hp-pdm-footer-copy">Back up, restore, or transfer your progress to another browser or device.</p><button class="hp-pdm-open" type="button">Manage Puzzle Progress</button></div>`;

    const overlay = document.createElement("div");
    overlay.className = "hp-pdm-overlay";
    overlay.setAttribute("aria-hidden", "true");
    overlay.innerHTML = `<section class="hp-pdm-modal" role="dialog" aria-modal="true" aria-labelledby="hp-pdm-title"><button class="hp-pdm-close" type="button" aria-label="Close"><span class="material-symbols-outlined" aria-hidden="true">close</span></button><h2 id="hp-pdm-title">Puzzle Progress Backup</h2><p class="hp-pdm-intro">Download your current puzzle progress or restore it in another browser or device. Only progress from the current puzzle platform is included.</p><div class="hp-pdm-actions"><button class="hp-pdm-btn primary" type="button" data-pdm-download>Download Progress Backup</button><button class="hp-pdm-btn secondary" type="button" data-pdm-import>Restore Progress Backup</button><input class="hp-pdm-file" type="file" accept=".json,application/json" data-pdm-file></div><div class="hp-pdm-status" role="status" aria-live="polite"></div><div class="hp-pdm-danger"><h3>Danger Zone</h3><p>Erase all current-platform puzzle progress stored in this browser. This cannot be undone without a downloaded backup.</p><button class="hp-pdm-danger-open" type="button">Erase All Puzzle Progress</button><div class="hp-pdm-confirm"><label>Type <strong>ERASE</strong> to confirm removal of all current puzzle progress and internal recovery copies.</label><input type="text" autocomplete="off" spellcheck="false" data-pdm-confirm-input><div class="hp-pdm-confirm-actions"><button class="hp-pdm-backup-first" type="button">Download a Backup First</button><button class="hp-pdm-erase" type="button" disabled>Erase All Puzzle Progress</button></div></div></div></section>`;
