/* =========================================================
   ...
   Updated: 2026-06-13 v1.7 — communication center, assist selection, and found-word visibility update

   Suggested filename:
   hare-word-search-platform-engine-v1.7.js
...
window.HareWordSearchEngine = {
  init({
    ...
    function getBoardCellClass(r, c) {
      ...
      if (selectedAssistWord) {
        const selectedPlacement = getPlacementForWord(selectedAssistWord);
        if (selectedPlacement && selectedPlacement.path.some(pos => pos.r === r && pos.c === c)) {
          classes.push("is-selected-word-path");
        }
      }
      ...
      listEl.innerHTML = normalizedWords
        ...
        .map(word => {
          ...
          const icon = found
            ...
            : "";
          return `<button type="button" class="${classes.join(" ")}" data-word="${escapeHtml(word)}">${icon}<span class="hp-ws-word-text">${escapeHtml(word)}</span></button>`;
        })
        .join("");
