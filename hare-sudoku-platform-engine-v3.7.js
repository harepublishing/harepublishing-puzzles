  ...
  init({
    ...
    function renderOverlayNextPanel() {
      ...
      if (alternate && !nextPreferred) {
        ...
        const remainingCopy = hasRemainingModes ? `${escapeHtml(joinList(remainingModes))} ${remainingModes.length === 1 ? "is" : "are"} still available in this Sudoku puzzle set.` : "Another challenge is still available.";
        overlayNextPanelEl.innerHTML = `
          <div class="hp-sudoku-next-panel-title">You're caught up on ${escapeHtml(preferred)}!</div>
          <p>Check back tomorrow for the next ${escapeHtml(preferredLower)} puzzle.</p>
          <p>${remainingCopy}</p>
      ...
      if (nextPreferred) {
        ...
        overlayNextPanelEl.innerHTML = `
          <div class="hp-sudoku-next-panel-title">${isProgress ? "Continue where you left off" : "Next for you"}</div>
          ${hasRemainingModes ? `<p>${escapeHtml(joinList(remainingModes))} ${remainingModes.length === 1 ? "is" : "are"} still available in this Sudoku puzzle set.</p>` : ""}
          <div class="hp-sudoku-next-panel-actions">
            <button class="hp-sudoku-next-panel-btn primary" type="button" data-a="request-next-sudoku" data-next-kind="recommended">${escapeHtml(label)}</button>
          </div>
        `;
        overlayNextPanelEl.hidden = false;
        return;
      ...
      if (!anyUnfinished) {
        overlayNextPanelEl.innerHTML = `
          <div class="hp-sudoku-next-panel-title">You're all caught up!</div>
          <p>Explore more puzzles in the Puzzlers Hub.</p>
          <div class="hp-sudoku-next-panel-actions">
    ...
    function renderOverlayContent() {
      ...
      const remainingModes = getRemainingModeLabels();
      const remainingText = remainingModes.length
        ? ` ${joinList(remainingModes)} ${remainingModes.length === 1 ? "is" : "are"} still available in this Sudoku set.`
        : " You have completed every level in this Sudoku set.";

      const puzzleSetTitle = cfg.puzzleId ? `Sudoku Puzzle #${cfg.puzzleId}` : "Sudoku Puzzle";

      if (state.solved) {
        overlayIconEl.textContent = "celebration";
        overlayTitleEl.textContent = displayName;
        overlayTextEl.textContent = `${puzzleSetTitle} status is shown above.${remainingText}`;
     ...
     <div class="hp-puzzle-date">
        ...
        <div class="hp-overlay" id="hp-overlay" aria-hidden="true">
          <div class="hp-modal" role="dialog" aria-modal="true" aria-label="Puzzle complete">
            ...
            <div class="hp-modal-actions">
              <a class="hp-link-btn primary-action" href="${escapeHtml(LINK_MORE_ONLINE)}">More Online Puzzles</a>
              <a class="hp-link-btn secondary-action" href="${escapeHtml(LINK_SHOP)}">Get Puzzle Books</a>

              <button class="hp-link-btn share-action" data-a="share">Share This Puzzle</button>
              <button class="hp-link-btn neutral-action" data-a="close-solved">Back to Puzzle</button>

              <button class="hp-link-btn danger-action" data-a="reset-board">Reset Puzzle</button>
            </div>
      ...
      mountClickHandler = (e) => {
        ...
        if (a === "request-next-sudoku") {
          ...
          if (typeof window.HareSudokuRequestNextSudoku === "function") {
            window.HareSudokuRequestNextSudoku({ source: "overlay", kind: target.dataset.nextKind || "recommended" });
          }
          return;
        }
