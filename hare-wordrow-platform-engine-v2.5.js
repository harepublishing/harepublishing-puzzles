...
window.HareWordrowEngine = (() => {
  const VERSION = "wordrow-platform-engine-v2.5";
  ...
  function renderRecommendationHtml(context = {}) {
    ...
    if (nextPuzzle?.puzzleId) {
      ...
      const messageTitle = context.solved
        ...
        : context.lost
          ? "Good try — another Wordrow is ready"
          : context.revealed
            ? "Answer revealed"
            : (isInProgress ? "Continue where you left off" : "Another Wordrow is ready");
      const messageCopy = isInProgress
        ? "Pick up where you left off and finish the hidden word."
        : "Keep your puzzle streak going with the next available Wordrow.";
    ...
    if (state.lost) {
      ...
      overlayTextEl.innerHTML = `
        <div class="hp-wordrow-overlay-next-copy">
          ...
          <span>Try another Wordrow puzzle and keep building your streak.</span>
        </div>
