...
window.HareSudokuPlatformEngine = {
  init({
    ...
    function renderOverlayNextPanel() {
      ...
      if (hasRemainingModes) {
        const remainingLabel = joinList(remainingModes);
        overlayNextPanelEl.innerHTML = `
          <div class="hp-sudoku-next-panel-title">Continue Solving</div>
          <p>${escapeHtml(remainingLabel)} ${remainingModes.length === 1 ? "is" : "are"} still available in Sudoku Puzzle #${escapeHtml(cfg.puzzleId || "")}. Keep going when you’re ready.</p>
          <div class="hp-sudoku-next-panel-actions">
            <button class="hp-sudoku-next-panel-btn secondary" type="button" data-a="close-solved">Continue This Sudoku</button>
            ${nextPuzzle && nextPuzzle.puzzleId ? `<button class="hp-sudoku-next-panel-btn" type="button" data-a="overlay-next-puzzle" data-puzzle-id="${escapeHtml(nextPuzzle.puzzleId)}">Play Your Next Sudoku</button>` : ""}
          </div>
        `;
      ...
      if (nextPuzzle && nextPuzzle.puzzleId) {
        overlayNextPanelEl.innerHTML = `
          <div class="hp-sudoku-next-panel-title">Play Your Next Sudoku</div>
          <p>You’ve completed every level in this Sudoku set. Your next available Sudoku puzzle is ready.</p>
