...
window.HareSudokuPlatformEngine = {
  init({
    ...
    if (container.dataset.hpSudokuMounted === "true") {
      console.warn("HareSudokuPlatformEngine: this container has already been mounted.");
      return;
    }
      ...
      return `
        <div class="hp-overlay-mode-summary" aria-label="Sudoku set status">
          ${availableModes.map(mode => {
            ...
            return `<span class="hp-overlay-mode-pill ${statusClass}"><strong>${escapeHtml(label)}:</strong> ${escapeHtml(status)}</span>`;
          }).join("")}
        </div>
      `;
    }
    ...
    function renderOverlayNextPanel() {
      ...
      const nextByMode = [];
      if (typeof window.HareSudokuGetNextPuzzleForMode === "function") {
        availableModes.forEach(mode => {
          try {
            const item = window.HareSudokuGetNextPuzzleForMode(mode);
            if (item && item.puzzleId) {
              nextByMode.push({ ...item, mode });
            }
          } catch (err) {
            console.warn(`Sudoku next ${mode} helper failed.`, err);
          }
        });
      }

      if (nextByMode.length) {
        overlayNextPanelEl.innerHTML = `
          <div class="hp-sudoku-next-panel-title">Play Your Next Puzzle</div>
          <p>Choose the next Sudoku level you want to play.</p>
          <div class="hp-sudoku-next-mode-buttons">
            ${nextByMode.map(item => {
              const modeLabel = getModeLabel(item.mode);
              return `<button class="hp-sudoku-next-panel-btn mode-${escapeHtml(item.mode)}" type="button" data-a="overlay-next-puzzle" data-puzzle-id="${escapeHtml(item.puzzleId)}" data-mode="${escapeHtml(item.mode)}">Next ${escapeHtml(modeLabel)}</button>`;
            }).join("")}
          </div>
        `;
      ...
      let nextPuzzle = null;
      if (typeof window.HareSudokuGetNextPuzzle === "function") {
        try {
          nextPuzzle = window.HareSudokuGetNextPuzzle();
        } catch (err) {
          console.warn("Sudoku next puzzle helper failed.", err);
        }
      }
      ...
      if (nextPuzzle && nextPuzzle.puzzleId) {
        overlayNextPanelEl.innerHTML = `
          <div class="hp-sudoku-next-panel-title">Play Your Next Puzzle</div>
          <p>Your next available Sudoku puzzle is ready. Try another level or keep your streak going.</p>
          <button class="hp-sudoku-next-panel-btn" type="button" data-a="overlay-next-puzzle" data-puzzle-id="${escapeHtml(nextPuzzle.puzzleId)}">Sudoku #${escapeHtml(nextPuzzle.puzzleId)}</button>
        `;
        overlayNextPanelEl.hidden = false;
        return;
      }
      ...
      overlayNextPanelEl.innerHTML = `
        ...
        <p>You’ve completed all available Sudoku puzzles. Check back for the next puzzle.</p>
      `;
      overlayNextPanelEl.hidden = false;
    }
    ...
    function renderOverlayContent() {
      ...
      if (state.solved) {
        ...
        overlayTitleEl.textContent = `You Solved ${displayName}!`;
      ...
      if (state.revealed) {
        ...
        overlayTitleEl.textContent = `Answer Revealed — ${displayName}`;
        overlayTextEl.textContent = `${puzzleSetTitle} status is shown above.${remainingText}`;
        overlayFooterEl.textContent = cfg.footer;
      }
    }
...
<style>
  ...
  #hp-sudoku-container.hp-sudoku #hp-overlay-icon {
    ...
    align-items: center !important;
    justify-content: center !important;
    font-variation-settings: 'FILL' 1, 'wght' 800, 'GRAD' 0, 'opsz' 40 !important;
    margin-bottom: 0 !important;
  }
  ...
  #hp-sudoku-container.hp-sudoku .hp-sudoku-next-panel-btn {
    border: 2px solid #e6c6f5 !important;
    border-radius: 999px !important;
    min-height: 36px !important;
  ...
  #hp-sudoku-container.hp-sudoku .hp-sudoku-next-panel-btn:hover,
  #hp-sudoku-container.hp-sudoku .hp-sudoku-next-panel-btn:focus-visible {
    ...
    function bindUIEvents() {
      ...
      mountClickHandler = (e) => {
        ...
        if (a === "overlay-next-puzzle") {
          ...
          hideOverlay();
          if (nextId && String(nextId) === String(currentSharedPuzzleId) && nextMode) {
            switchMode(nextMode);
            return;
          }
          if (nextId && typeof window.HareSudokuLoadPuzzle === "function") {
            ...
            window.location.href = `${window.location.origin}${window.location.pathname}?puzzle=${encodeURIComponent(nextId)}`;
          }
          return;
        }
      };
