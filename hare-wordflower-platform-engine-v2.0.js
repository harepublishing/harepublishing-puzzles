...
window.HareWordFlowerEngine = (() => {
  const VERSION = "wordflower-platform-v1.9";
  ...
  const CSS = `
    ...
    #hp-wordflower-container .hpwf-current-word { min-height:34px; margin:0 auto 8px; display:flex; align-items:center; justify-content:center; font-size:clamp(24px,4vw,32px); line-height:1.1; letter-spacing:.08em; color:#111; font-weight:900; text-align:center; }
    #hp-wordflower-container .hpwf-current-word.placeholder { color:#b0b8c0; letter-spacing:.02em; font-size:19px; }

    #hp-wordflower-container .hpwf-flower-wrap { width:100%; display:flex; justify-content:center; align-items:center; margin:0 auto 10px; overflow:visible; }
    #hp-wordflower-container .hpwf-flower { position:relative; width:300px; height:300px; max-width:100%; flex:0 0 auto; margin:0 auto; }
    #hp-wordflower-container .hpwf-flower-core { position:absolute; left:82px; top:82px; width:136px; height:136px; border-radius:50%; background:radial-gradient(circle at 35% 30%,#fff 0%,#fff7f7 50%,#fff0f1 100%); border:10px solid #fff; box-shadow:0 0 0 12px rgba(246,141,145,.18), 0 14px 32px rgba(0,0,0,.10); z-index:4; pointer-events:none; }
    ...
    #hp-wordflower-container .hpwf-center-letter { left:90px; top:90px; width:120px; height:120px; background:#F68D91; color:#fff; border:10px solid #fff; z-index:5; box-shadow:0 0 0 10px rgba(246,141,145,.32), 0 16px 32px rgba(246,141,145,.35); font-size:34px; }
    ...
    #hp-wordflower-container .hpwf-outer-letter { width:80px; height:80px; z-index:2; }
    #hp-wordflower-container .hpwf-pos-0 { left:110px; top:2px; }
    #hp-wordflower-container .hpwf-pos-1 { left:190px; top:55px; }
    #hp-wordflower-container .hpwf-pos-2 { left:190px; top:165px; }
    #hp-wordflower-container .hpwf-pos-3 { left:110px; top:218px; }
    #hp-wordflower-container .hpwf-pos-4 { left:30px; top:165px; }
    #hp-wordflower-container .hpwf-pos-5 { left:30px; top:55px; }
    ...
    #hp-wordflower-container .hpwf-actions button, #hp-wordflower-container .hpwf-reveal-actions button { appearance:none; border-radius:14px; padding:9px 15px; font-size:14px; font-weight:900; font-family:inherit; cursor:pointer; border:2px solid #f7c7ca; background:#fff; color:#9d4147; transition:all .18s ease; }
    ...
    #hp-wordflower-container .hp-modal { width:min(760px,100%); max-height:calc(100% - 30px); overflow:auto; background:#fff; border-radius:24px; box-shadow:0 18px 52px rgba(0,0,0,.20); padding:18px 22px; text-align:center; }
    #hp-wordflower-container .hp-modal.master { width:min(720px,100%); }
    ...
    #hp-wordflower-container .hp-modal h3 { margin:8px 0 8px; font-size:30px; line-height:1.1; color:#9d4147; font-weight:900; }
    ...
    #hp-wordflower-container .hpwf-overlay-progress { margin:0 auto 12px; }
    ...
    #hp-wordflower-container .hp-modal-lead { color:#555; font-size:17px; line-height:1.35; font-weight:900; margin:8px 0; }
    #hp-wordflower-container .hp-modal-subtext { color:#555; font-size:15px; line-height:1.35; font-weight:700; margin:4px 0; }
    ...
    #hp-wordflower-container .hp-recommend-actions { display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-top:12px; }
    #hp-wordflower-container .hp-recommend-actions .hp-link-btn { min-height:48px; }
  ...
  function renderWordList(){
    ...
    return list.map(word=>`<div class="hpwf-word-item ${revealedSet.has(word)?"revealed":""} ${isPangram(word)?"pangram":""} ${state.wordAchievements && state.wordAchievements[word]?"achievement":""}">
      <div class="hpwf-word-text">${wordAchievementIcon(word)}${escapeHtml(word)}</div>
  ...
  function renderMasterContinueHtml(){
    const next=findNextPlayableWordFlower({ excludePuzzleId:puzzle?.puzzleId, excludeFinished:true });
    const tryAnother = next
      ? `<button class="hp-link-btn" type="button" data-a="load-puzzle" data-puzzle-id="${escapeHtml(next.id)}">Try Another Word Flower</button>`
      : `<a class="hp-link-btn" href="${escapeHtml(MORE_PUZZLES_URL)}">More Online Puzzles</a>`;
    return `<div class="hp-recommend-card">
      <div class="hp-recommend-title">Keep Going?</div>
      <div class="hp-recommend-copy">You’ve reached Word Flower Success! Keep searching to find every word and complete the puzzle.</div>
      <div class="hp-recommend-actions">
        <button class="hp-link-btn primary" type="button" data-a="close-overlay">Continue This Puzzle</button>
        ${tryAnother}
      </div>
    </div>`;
  }
  ...
  function showOverlay(kind){
    ...
    const modal=overlay.querySelector(".hp-modal");
    if(modal) modal.classList.toggle("master", kind==="master");

    if(kind==="master"){
      icon.textContent="home_and_garden";
      title.textContent="Master Gardener Achieved!";
      text.innerHTML=renderMasterContinueHtml();
    ...
    meta.innerHTML = kind === "master" ? "" : getOverlayStatsLine();
    meta.style.display = kind === "master" ? "none" : "flex";
