/* =========================================================
   ...
   - v2.9: JSON-LD Game schema added for SEO/AIO consistency
   - v3.0: compact square character cards on mobile to reduce scrolling
   - v3.1: compact silent-character reactions in the conversation area
   - v3.2: finished levels retain reporting fields but restore answers from puzzle JSON
   ========================================================= */
...
window.HareKnightsKnavesPlatformEngine = {
  init({
    ...
    function injectSchema() {
      const schemaId = "hp-schema-knights-knaves";
      ...
      const accessConfig = window.HareKnightsKnavesAccessConfig || {};
      const collectionPath = accessConfig.publicPlayUrl || window.HareKnightsKnavesPlayUrl || pageData?.publicPlayUrl || pageData?.playPageUrl || "/knights-knaves";
      const collectionUrl = new URL(collectionPath, window.location.origin).toString();
      const nowYear = new Date().getFullYear();
      const schemaName = pageData?.puzzleTitle || (puzzleId ? `Knights & Knaves Puzzle #${puzzleId}` : "Knights & Knaves Puzzle");
      const levelNames = availableModes.map(mode => MODE_META[mode]?.label).filter(Boolean).join(", ");
      const schemaDescription = puzzleId
        ? `Play Knights & Knaves Puzzle #${puzzleId} online from Hare Publishing. Solve the logic statements, identify each character, and save your progress${levelNames ? ` across ${levelNames} levels` : ""}.`
        : "Play Knights & Knaves online from Hare Publishing. Solve the logic statements, identify each character, and save your progress.";
      ...
      const schemaData = {
        ...
        "keywords": ["Knights and Knaves", "logic puzzle", "truth teller puzzle", "daily puzzle", "online puzzle", "Hare Publishing"],
        "audience": {
          "@type": "PeopleAudience",
          "suggestedMinAge": "8"
        },
    ...
    function normalizeLevel(level, mode) {
      ...
      const silentReaction = level.silentReaction && typeof level.silentReaction === "object"
        ? {
            person:String(level.silentReaction.person || "").trim().toUpperCase(),
            text:String(level.silentReaction.text || "").trim()
          }
        : null;
      return { ...MODE_META[mode], people, letters, statements, solution, silentReaction };
    }
    ...
    function getBackupKey(mode = currentMode) {
      const saveKey = getSaveKey(mode);
      const progressCore = window.HarePuzzleProgressCore;
      return progressCore && typeof progressCore.backupKey === "function"
        ? progressCore.backupKey(saveKey)
        : `hp_backup__${saveKey}`;
    }

    function isFinishedState(state) {
      return Boolean(state && (state.solved || state.revealed || state.solvedAt || state.revealedAt));
    }

    function normalizeFinishedState(state) {
      const revealed = Boolean(state.revealed || state.revealedAt);
      const solved = Boolean(!revealed && (state.solved || state.solvedAt));
      state.solved = solved;
      state.revealed = revealed;
      state.solvedAt = solved ? String(state.solvedAt || state.updatedAt || "") : "";
      state.revealedAt = revealed ? String(state.revealedAt || state.updatedAt || "") : "";
      return state;
    }

    function getPersistableState(state) {
      if (!isFinishedState(state)) return state;
      normalizeFinishedState(state);
      return {
        solved:state.solved,
    ...
    function writeStoredState(mode, state) {
      const saveKey = getSaveKey(mode);
      const persistable = getPersistableState(state);
      let stored = false;
      if (Core && typeof Core.setSavedState === "function") {
        stored = Core.setSavedState(saveKey, persistable, { allowDowngrade:isFinishedState(state) });
      } else {
        localStorage.setItem(saveKey, JSON.stringify(persistable));
        stored = true;
      }
      if (stored && isFinishedState(state)) localStorage.removeItem(getBackupKey(mode));
      return stored;
    }

    function loadState(mode) {
      try {
        const parsed = Core && typeof Core.getSavedState === "function"
          ? Core.getSavedState(getSaveKey(mode), null)
          : JSON.parse(localStorage.getItem(getSaveKey(mode)) || "null");
        if (!parsed) return defaultState();
        const state = { ...defaultState(), ...parsed, answers:{ ...(parsed.answers || {}) } };
        if (isFinishedState(state)) {
          normalizeFinishedState(state);
          state.answers = { ...(PUZZLES[mode].solution || {}) };
          state.evaluated = true;
          writeStoredState(mode, state);
        }
        return state;
    ...
    function saveState(mode = currentMode, action = "save") {
      ...
      try { writeStoredState(mode, state); }
    ...
    function renderStatements(animateMessages = false) {
      ...
      const messages = statements.map(statement => ({...statement, silent:false}));
      if (cfg.silentReaction?.person && cfg.silentReaction?.text) {
        const silentPerson = cfg.silentReaction.person;
        const alreadySpeaks = statements.some(statement => statement.speaker === silentPerson);
        if (cfg.letters.includes(silentPerson) && !alreadySpeaks) {
          messages.push({
            speaker:silentPerson,
            text:cfg.silentReaction.text,
            silent:true
          });
        }
      }
      messages.sort((left, right) =>
        cfg.letters.indexOf(left.speaker) - cfg.letters.indexOf(right.speaker)
      );
      return `<div class="hp-knights-statements">
        <div class="hp-knights-message-list">
          ${messages.map((st, index) => {
            ...
            return `<div class="hp-knights-message hp-knights-message-${side} ${st.silent ? "hp-knights-message-silent" : ""} ${animateMessages ? "animate" : ""}" ${animateMessages ? `style="animation-delay:${index * 520}ms;"` : ""}>
              ...
              <div class="hp-knights-message-bubble">
                ${st.silent
                  ? `<div class="hp-knights-message-name"><span class="hp-knights-silent-label">No statement</span></div>
    ...
    function renderUI({ playMessages = allowNextMessageSound } = {}) {
      ...
      mount.innerHTML = `<style>
        ...
        #hp-knights-container .hp-knights-message-silent{opacity:.88;}
        #hp-knights-container .hp-knights-message-silent .hp-knights-message-avatar{width:52px;height:52px;margin:0 auto;filter:saturate(.72);}
        #hp-knights-container .hp-knights-message-silent .hp-knights-message-bubble{min-height:52px;border-style:dashed;background:#f8fafc;color:#64748b;}
        #hp-knights-container .hp-knights-message-silent .hp-knights-message-text{font-size:13px;font-style:italic;font-weight:700;}
        #hp-knights-container .hp-knights-silent-label{display:inline-flex;border-radius:999px;background:#e2e8f0;color:#475569;padding:2px 7px;font-size:10px;line-height:1.2;font-weight:900;text-transform:uppercase;letter-spacing:.04em;}
        ...
        @media(max-width:760px){#hp-knights-container .hp-mount{display:flex;flex-direction:column;}#hp-knights-container .hp-mount>.hp-knights-top{order:1;}#hp-knights-container .hp-mount>.hp-knights-completion-status{order:2;}#hp-knights-container .hp-mount>.hp-knights-statements{order:3;}#hp-knights-container .hp-mount>.hp-knights-answer-grid{order:4;}#hp-knights-container .hp-mount>.hp-knights-status{order:5;}#hp-knights-container .hp-mount>.hp-knights-actions{order:6;}#hp-knights-container .hp-mount>.hp-knights-logic-panel{order:7;}#hp-knights-container .hp-knights-modal{width:min(720px,100%);padding:22px 16px;}#hp-knights-container .hp-knights-overlay-stat-row{flex-wrap:wrap;gap:6px 12px;}#hp-knights-container .hp-knights-overlay-mode-summary{flex-wrap:wrap;}#hp-knights-container .hp-knights-top{display:block;}#hp-knights-container .hp-knights-mode-switch{max-width:none;margin-bottom:8px;}#hp-knights-container .hp-knights-status{font-size:15px;line-height:1.25;}#hp-knights-container .hp-knights-statements{padding:11px 10px;}#hp-knights-container .hp-knights-message{grid-template-columns:58px auto;max-width:100%;}#hp-knights-container .hp-knights-message-right{grid-template-columns:auto 58px;}#hp-knights-container .hp-knights-message-avatar{width:58px;height:58px;}#hp-knights-container .hp-knights-message-bubble{min-height:58px;}#hp-knights-container .hp-knights-answer-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;}#hp-knights-container .hp-knights-answer-grid .hp-knights-person-card:last-child:nth-child(odd){grid-column:1 / -1;width:calc(50% - 4px);justify-self:center;}#hp-knights-container .hp-knights-photo-wrap{aspect-ratio:1/1;}#hp-knights-container .hp-knights-actions{grid-template-columns:repeat(2,minmax(0,1fr));}#hp-knights-container .hp-knights-overlay-mode-summary{flex-wrap:wrap;}}
      </style>
    ...
    function resetLevel() {
      ...
      try {
        const saveKey = getSaveKey(currentMode);
        if (Core && typeof Core.removeSavedState === "function") Core.removeSavedState(saveKey, { allowDowngrade:true });
        else localStorage.removeItem(saveKey);
        localStorage.removeItem(getBackupKey(currentMode));
      } catch {}
