/* =========================================================
   ...
   Suggested filename:
   hare-regular-sudoku-engine-v3.0.js
...
window.HareRegularSudokuEngine = {
  init({
    ...
    if (!availableModes.length) {
      showConfigError("At least one Sudoku puzzle is required. Add daily, easy, medium, hard, or extreme puzzle data.");
      return;
    }
    ...
    function applyTestMobileKeypadSizing() {
      ...
      mount.querySelectorAll(".hp-btn-grid .hp-btn").forEach(btn => {
        if (shouldShorten) {
          ...
          ["width", "min-width", "height", "min-height", "max-height", "padding", "display", "align-items", "justify-content", "line-height", "font-size", "aspect-ratio"].forEach(prop => {
