window.HareCryptogramEngine = {
  init({ containerId = "hp-cryptogram-container", dataObject } = {}) {
    ...
    function getCryptogramStats() {
      const items = getStoredCryptogramItems();
      const solved = items.filter(item => isSolvedOnly(item.saved));
      const revealed = items.filter(item => isRevealedOnly(item.saved));
      const played = items.filter(item => isSolvedOnly(item.saved) || isRevealedOnly(item.saved));

      const finishedDates = new Set();
      played.forEach(item => {
        const raw = getFinishedDate(item.saved);
        if (!raw) return;
        const d = new Date(raw);
        if (!isNaN(d)) finishedDates.add(localDateKey(d));
      });
    ...
    function getOverlayStatsLine() {
      ...
      return `
        <span class="hp-result-stat-chip"><span class="material-symbols-outlined" aria-hidden="true">local_fire_department</span><strong>${stats.streak.toLocaleString()}</strong><span>Day Streak</span></span>
        <span class="hp-result-stat-chip"><span class="material-symbols-outlined" aria-hidden="true">trophy</span><strong>${stats.solved.toLocaleString()}</strong><span>Solved</span></span>
        <span class="hp-result-stat-chip"><span class="material-symbols-outlined" aria-hidden="true">visibility</span><strong>${stats.revealed.toLocaleString()}</strong><span>Revealed</span></span>
        <span class="hp-result-stat-chip"><span class="material-symbols-outlined" aria-hidden="true">beenhere</span><strong>${stats.played.toLocaleString()}</strong><span>Played</span></span>
      `;
    }
    ...
    function getPlayUrl(puzzleId) {
      const base = window.HareCryptogramPlayUrl || window.location.pathname || "/cryptogram";
      ...
      if (nextPuzzle) {
        return `
          <div class="hp-recommend-card">
            ...
            <a class="hp-link-btn primary full" href="${escapeHtml(getPlayUrl(nextPuzzle.id))}">
        ...
        <div class="hp-crypto-shell">
          <div class="hp-crypto-card">
            ...
            <div class="hp-modal-actions">
              ...
              <button class="hp-link-btn secondary hp-start-over-btn" data-a="start-over">Start Over</button>
            </div>
