...
window.HareWordFlowerEngine = (() => {
  const VERSION = "wordflower-platform-v1.6";
  ...
  const CSS = `
    ...
    #hp-wordflower-container .hpwf-card { background:transparent; border:0; box-shadow:none; padding:0; position:relative; }
    ...
    #hp-wordflower-container .hpwf-main-panel { overflow:visible; min-height:685px; height:685px; display:flex; flex-direction:column; }
    #hp-wordflower-container .hpwf-word-panel { display:flex; flex-direction:column; overflow:hidden; min-height:685px; height:685px; max-height:685px; }
    ...
    #hp-wordflower-container .hpwf-word-list { flex:1 1 auto; min-height:0; max-height:100%; overflow-y:auto; padding-right:6px; scroll-behavior:auto; overscroll-behavior:contain; }
    ...
    #hp-wordflower-container .hp-overlay { display:none; position:absolute; inset:0; z-index:50; background:rgba(255,255,255,.78); align-items:flex-start; justify-content:center; padding:18px; border-radius:20px; }
    #hp-wordflower-container .hp-overlay.on { display:flex; }
    #hp-wordflower-container .hp-modal { width:min(560px,100%); max-height:none; overflow:visible; background:#fff; border-radius:22px; box-shadow:0 18px 48px rgba(0,0,0,.18); padding:24px; text-align:center; }
    #hp-wordflower-container .hp-modal .material-symbols-outlined.hp-result-icon { font-size:30px; color:#9d4147; font-variation-settings:'FILL' 1,'wght' 600,'GRAD' 0,'opsz' 40; }
    #hp-wordflower-container .hp-modal h3 { margin:8px 0 10px; font-size:28px; line-height:1.1; color:#9d4147; font-weight:900; }
    ...
    #hp-wordflower-container .hpwf-overlay-progress { margin:0 auto 14px; }
    #hp-wordflower-container .hpwf-overlay-progress .hpwf-progress-card { min-height:auto; padding:10px; box-shadow:none; }
    #hp-wordflower-container .hpwf-overlay-progress .hpwf-progress-top { grid-template-columns:minmax(118px,auto) minmax(0,1fr) auto; gap:8px; }
    #hp-wordflower-container .hpwf-overlay-progress .hpwf-current-icon{width:30px;height:30px;}
    #hp-wordflower-container .hpwf-overlay-progress .hpwf-current-icon .material-symbols-outlined{font-size:20px;}
    #hp-wordflower-container .hpwf-overlay-progress .hpwf-level-label{font-size:16px;}
    #hp-wordflower-container .hpwf-overlay-progress .hpwf-level-sub{font-size:10px;}
    #hp-wordflower-container .hpwf-overlay-progress .hpwf-progress-percent{font-size:23px;}
    #hp-wordflower-container .hpwf-overlay-progress .hpwf-progress-stat{padding:5px 4px;}
    ...
    #hp-wordflower-container .hpwf-overlay-progress .hpwf-vine{padding-top:8px;}
    #hp-wordflower-container .hpwf-overlay-progress .hpwf-vine::before{top:25px;}
    #hp-wordflower-container .hpwf-overlay-progress .hpwf-vine-fill{top:25px;}
    #hp-wordflower-container .hpwf-overlay-progress .hpwf-node-dot{width:28px;height:28px;}
    #hp-wordflower-container .hpwf-overlay-progress .hpwf-node.current .hpwf-node-dot{width:34px;height:34px;}
    #hp-wordflower-container .hpwf-overlay-progress .hpwf-node-dot .material-symbols-outlined{font-size:18px;}
    #hp-wordflower-container .hpwf-overlay-progress .hpwf-node-label{display:none;}

    #hp-wordflower-container .hp-modal-lead { color:#555; font-size:16px; line-height:1.35; font-weight:900; margin:8px 0; }
    #hp-wordflower-container .hp-modal-subtext { color:#555; font-size:14px; line-height:1.35; font-weight:700; margin:4px 0; }
    #hp-wordflower-container .hp-recommend-card { margin:14px auto 0; padding:16px; border:1px solid #f7c7ca; background:#fff5f6; border-radius:18px; }
    #hp-wordflower-container .hp-recommend-title { font-size:18px; line-height:1.15; font-weight:900; color:#9d4147; margin:0 0 8px; }
    #hp-wordflower-container .hp-recommend-copy { color:#555; font-size:14px; line-height:1.35; font-weight:600; margin:0 0 12px; }
    #hp-wordflower-container .hp-link-btn.full { width:100%; max-width:260px; margin-left:auto; margin-right:auto; }

    #hp-wordflower-container .hp-modal-actions { display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-top:16px; }
  ...
  function achievementWords(){
    if(state.revealAllUsed && Array.isArray(state.achievementFoundAtRevealAll)) return uniqueWords(state.achievementFoundAtRevealAll).filter(w=>allWords().includes(w));
    return foundWords();
  }
  function revealedWords(){ return uniqueWords(state.revealedWords).filter(w=>allWords().includes(w)); }
  function foundCount(){ return achievementWords().length; }
  ...
  function currentScore(){ return achievementWords().reduce((sum,w)=>sum+scoreWord(w),0); }
  function totalScore(){ return allWords().reduce((sum,w)=>sum+scoreWord(w),0); }
  function pangrams(){ return allWords().filter(isPangram); }
  function pangramsFound(){ return achievementWords().filter(isPangram); }
  ...
  function normalizeState(){
    ...
    if(state.revealAllUsed){
      ...
      if(state.completed) state.status = "complete";
  ...
  function renderWordList(){
    ...
    const displayWords = state.revealAllUsed ? uniqueWords([...found, ...revealedSet]) : found;
    if(!displayWords.length){
      return `<div class="hpwf-empty">No words found yet. Start building!</div>`;
    }
    const list=[...displayWords].sort((a,b)=>a.length-b.length||a.localeCompare(b));
  ...
  function revealAll(){
    ...
    state.achievementFoundAtRevealAll = foundWords();
    state.achievementScoreAtRevealAll = currentScore();
    state.achievementPctAtRevealAll = completionPct();
    ...
    state.current=""; state.status="revealed"; state.masterGardener=false; state.solved=false; state.completed=false;
