...
window.HareKnightsKnavesPlatformEngine = {
  init({
    ...
    function getOverlayNextActionsHtml() {
      if (typeof window.HareKnightsGetNextPuzzleOptions !== "function") {
        return typeof window.HareKnightsRequestNextPuzzle === "function"
          ? `<button type="button" class="hp-knights-primary" data-a="request-next">Play Your Next Puzzle</button>`
          : `<a class="hp-knights-primary" href="/puzzlers-hub">Puzzlers Hub</a>`;
      }
      ...
      if (options?.preferredLevelExhausted && options?.alternatePuzzle) {
        ...
        return `<span class="hp-knights-caught-up">You're caught up on ${escapeHtml(preferred)}!<br><small>Check back tomorrow for the next ${escapeHtml(preferred)} puzzle.</small></span><button type="button" class="hp-knights-primary" data-a="request-next" data-next-kind="alternate">Try ${escapeHtml(altMode)} #${escapeHtml(alt.puzzleId)}</button>`;
      }
      if (options?.allDone) {
        return `<span class="hp-knights-caught-up">You're all caught up! Check back tomorrow.</span><a class="hp-knights-primary" href="/puzzlers-hub">Puzzlers Hub</a>`;
      }
      return `<span class="hp-knights-caught-up">You're all caught up! Check back tomorrow.</span>`;
    }
    ...
    function renderUI({ playMessages = allowNextMessageSound } = {}) {
      ...
      mount.innerHTML = `<style>
        ...
        #hp-knights-container .hp-knights-modal-actions{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:8px;margin-top:10px;}
        #hp-knights-container .hp-knights-modal-actions button,#hp-knights-container .hp-knights-modal-actions a,#hp-knights-container .hp-knights-caught-up{border:2px solid transparent;border-radius:12px;min-height:36px;padding:8px 10px;font-weight:900;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;justify-content:center;font-family:inherit;font-size:12px;line-height:1.1;}#hp-knights-container .hp-knights-caught-up{grid-column:1 / -1;cursor:default;color:#555;background:#f7f9fb;border-color:#dde7ef;flex-direction:column;gap:3px;}#hp-knights-container .hp-knights-caught-up small{font-size:11px;font-weight:800;color:#666;}
