...
window.HareWordScramblePlatformEngine = (() => {
  const VERSION = "word-scramble-platform-engine-v1.2";
  ...
  const CSS = `
    ...
    @media(max-width:560px){
      ...
      #hp-wordscramble-container .hpwsc-slot,
      #hp-wordscramble-container .hpwsc-letter { width:clamp(28px,8.4vw,38px); min-width:0; flex:0 1 38px; height:45px; border-radius:12px; font-size:clamp(18px,6vw,22px); }
  ...
  function getCurrentEntry(){
    ...
    const explicit=puzzleData.find(item=>item.id===state.currentWordId && !completed.has(item.answer));
  ...
  function statusMessage(){
    ...
    if (state.revealed) return "Word Scramble completed with revealed words.";
    if (!completedCount() && !state.currentGuess) return "Choose a scrambled word and tap letters to build the answer.";
  ...
  function chooseWord(id){
    ...
    if(completedSet().has(item.answer)) return;
  ...
  function submitGuess(){
    ...
    state.currentGuess=""; state.usedLetterIds=[];
    const next=puzzleData.find(item=>!completedSet().has(item.answer));
  ...
  function revealWord(){
    ...
    if(!confirm(`Reveal the answer for ${entry.scrambled}?`)) return;
    ...
    else flash(`Revealed ${entry.answer}.`);
  }
  ...
  function renderOverlayContent(){
    ...
    if(state.solved){
      ...
      overlayTextEl.innerHTML=`<div class="hp-modal-lead">You completed this Word Scramble with one or more revealed words.</div>${renderRecommendationHtml()}`;
    }
  }
  ...
  function renderCurrent(){
    ...
    return `<div class="hpwsc-current">
      <h3 class="hpwsc-current-title">${state.revealed||solved||revealed ? escapeHtml(entry.answer) : escapeHtml(entry.scrambled)}</h3>
      <div class="hpwsc-hint-buttons" aria-label="Word hints">
        <button type="button" class="hpwsc-btn hint-toggle ${showHint1?"is-on":""}" data-a="toggle-hint" data-hint="1">Hint 1</button>
        <button type="button" class="hpwsc-btn hint-toggle ${showHint2?"is-on":""}" data-a="toggle-hint" data-hint="2">Hint 2</button>
      </div>
      <div class="hpwsc-hint-output">
        <div class="hpwsc-clue-box ${showHint1?"":"is-hidden"}"><strong>Hint 1:</strong> ${escapeHtml(hint1)}</div>
        <div class="hpwsc-clue-box ${showHint2?"":"is-hidden"}"><strong>Hint 2:</strong> ${escapeHtml(hint2)}</div>
      </div>
  ...
  function init(options={}){
    ...
    puzzleData=puzzleData.map(item=>{ const chars=item.answer.split(""); let scrambled=item.answer; let tries=0; while(scrambled===item.answer && tries<25){ scrambled=shuffleDeterministic(chars, `${puzzle.puzzleId}|${item.answer}|${tries}`).join(""); tries++; } return {...item, scrambled, length:item.answer.length}; }).sort((a,b)=>a.scrambled.localeCompare(b.scrambled));
