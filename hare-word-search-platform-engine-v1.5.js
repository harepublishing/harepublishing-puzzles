/* =========================================================
   ...
   Updated: 2026-06-13 v1.5 — Assist tools, Help fix, and release update
...
window.HareWordSearchEngine = {
  init({
    ...
    function selectAssistWord(word) {
      if (isFinished()) return;
      selectedAssistWord = cleanWord(word);
    ...
    function renderWordListOnly() {
      ...
      listEl.innerHTML = normalizedWords
        ...
        .map(word => {
          ...
          if (selectedAssistWord === word) classes.push("is-selected-assist");
          const check = found ? `<span class="hp-ws-word-check" aria-hidden="true">✓</span>` : "";
          return `<button type="button" class="${classes.join(" ")}" data-word="${escapeHtml(word)}"><span class="hp-ws-word-text">${escapeHtml(word)}</span>${check}</button>`;
        })
        .join("");

      listEl.querySelectorAll("[data-word]").forEach(btn => {
        btn.addEventListener("click", () => selectAssistWord(btn.getAttribute("data-word")));
      });
    }
          ...
          <div class="hp-ws-col-right">
            <div class="hp-ws-panel">
              <div class="hp-puzzle-tools" aria-label="Word Search puzzle controls">
                <details class="hp-ws-assist" id="hp-ws-assist-menu">
                  <summary class="hp-tool-btn assist-toggle">Assist</summary>
                  <div class="hp-ws-assist-panel">
                    ...
                    <button type="button" class="hp-tool-btn assist-option" data-a="assist-direction">Word Direction</button>
                    <button type="button" class="hp-tool-btn assist-option" data-a="assist-first-letter">Show First Letter</button>
                  </div>
                </details>
                <button type="button" class="hp-tool-btn clear-tool" data-a="clear-selection">Clear</button>
              </div>
        ...
        <div class="hp-overlay" id="hp-ws-overlay" aria-hidden="true">
          <div class="hp-modal" role="dialog" aria-modal="true" aria-label="Word search result">
            ...
            <div class="hp-modal-actions">
              ...
              <button class="hp-link-btn full danger" data-a="reset-puzzle">Start Over</button>
            </div>
