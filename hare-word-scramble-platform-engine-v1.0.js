...
window.HareWordScramblePlatformEngine = (() => {
  const VERSION = "word-scramble-platform-engine-v1.0";
  ...
  const CSS = `
    ...
    #hp-wordscramble-container .hpwsc-status {
      width:100%;
      max-width:900px;
      margin:0 auto 16px;
      padding:10px 14px;
    ...
    #hp-wordscramble-container .hpwsc-status-msg {
      display:block;
      width:100%;
      font-size:14px;
      font-weight:900;
    ...
    #hp-wordscramble-container .hpwsc-current-title {
      text-align:center;
      color:#c86e00;
      margin:4px 0 12px;
      font-size:20px;
    ...
    #hp-wordscramble-container .hp-result-puzzle-title { display:block; color:#c86e00; font-size:19px; line-height:1.2; font-weight:900; margin-bottom:12px; }
    ...
    #hp-wordscramble-container .hp-result-stat-chip .material-symbols-outlined { font-size:16px; color:#c86e00; }
    #hp-wordscramble-container .hp-result-stat-chip strong { color:#c86e00; font-weight:900; }
    ...
    #hp-wordscramble-container .hp-recommend-title { font-size:14px; font-weight:900; color:#c86e00; margin-bottom:4px; }
    ...
    #hp-wordscramble-container .hp-link-btn.neutral:hover { background:#f7941c; color:#fff; border-color:#f7941c; transform:translateY(-1px); }
    #hp-wordscramble-container .hp-link-btn.share { background:#fff9f0; color:#c86e00; border-color:#ffe1b8; box-shadow:0 4px 12px rgba(247,148,28,.14); }
    #hp-wordscramble-container .hp-link-btn.share:hover { background:#f7941c; color:#fff; border-color:#f7941c; transform:translateY(-1px); }
    ...
    @media(max-width:900px){
      ...
      #hp-wordscramble-container .hpwsc-word-list { max-height:none; }
    }

    @media(max-width:560px){
      #hp-wordscramble-container .hpwsc-panel { padding:13px; }
      #hp-wordscramble-container .hpwsc-slot,
      #hp-wordscramble-container .hpwsc-letter { width:38px; min-width:38px; height:45px; border-radius:12px; font-size:22px; }
  ...
  function injectStyles(){
    if (document.getElementById("hp-word-scramble-platform-engine-css-v10")) return;
    const style = document.createElement("style");
    style.id = "hp-word-scramble-platform-engine-css-v10";
  ...
  function defaultState(){
    return { solvedWords:[], currentWordId:"", currentGuess:"", usedLetterIds:[], solved:false, revealed:false, status:"in-progress", solvedAt:"", completedAt:"", revealedAt:"", updatedAt:"", lastPlayedAt:"", startedAt:"" };
  }
  function readState(){
  ...
  function remainingCount(){ return Math.max(0, puzzleData.length - solvedCount()); }
  function progressPercent(){ return puzzleData.length ? (solvedCount()/puzzleData.length)*100 : 0; }
  function isAllSolved(){ return puzzleData.length > 0 && solvedCount() >= puzzleData.length; }
  ...
  function getCurrentEntry(){
    ...
    const explicit=puzzleData.find(item=>item.id===state.currentWordId && !solved.has(item.answer));
    if (explicit) return explicit;
    const next=puzzleData.find(item=>!solved.has(item.answer));
  ...
  function statusMessage(){
    ...
    if (state.revealed) return "Answers revealed. Back to review the completed puzzle.";
    if (!solvedCount() && !state.currentGuess) return "Choose a scrambled word and tap letters to build the answer.";
    const entry=getCurrentEntry();
    if (!entry) return "Word Scramble ready.";
    if (state.currentGuess.length < entry.answer.length) return `${entry.answer.length - state.currentGuess.length} letter${entry.answer.length - state.currentGuess.length === 1 ? "" : "s"} left.`;
    return "Submit your answer when you're ready.";
  }
  ...
  function recomputeState(){
    ...
    if (!state.revealed && isAllSolved()) {
      state.solved=true; state.completed=true; state.status="solved";
      if(!state.solvedAt) state.solvedAt=new Date().toISOString();
      if(!state.completedAt) state.completedAt=state.solvedAt;
    }
    if (state.revealed) { state.solved=false; state.completed=false; state.status="revealed"; }
  }
  ...
  function chooseWord(id){
    ...
    if(solvedSet().has(item.answer)) return;
    state.currentWordId=id; state.currentGuess=""; state.usedLetterIds=[];
  ...
  function submitGuess(){
    ...
    if(state.solved) showOverlay(); else flash(`Correct! ${entry.answer} solved.`);
  }
  ...
  function revealAnswers(){
    ...
    if(!confirm("Reveal all answers for this Word Scramble puzzle?")) return;
    state.solvedWords=puzzleData.map(item=>item.answer);
    state.currentGuess=""; state.usedLetterIds=[];
    state.solved=false; state.completed=false; state.revealed=true; state.status="revealed"; state.revealedAt=new Date().toISOString();
    updateAfterChange("revealed", true);
    showOverlay();
  }
  ...
  function renderOverlayContent(){
    ...
    if(state.solved){
      ...
      overlayTextEl.innerHTML=`<div class="hp-modal-lead">The answers are now shown in the puzzle.</div>${renderRecommendationHtml()}`;
    }
  }
  ...
  function renderCurrent(){
    ...
    const guessDisplay=(state.revealed||solved) ? entry.answer : state.currentGuess;
    const letters=currentLetters();
    const disabled=isEnded()||solved;
    return `<div class="hpwsc-current">
      <h3 class="hpwsc-current-title">${state.revealed||solved ? escapeHtml(entry.answer) : escapeHtml(entry.scrambled)}</h3>
      <div class="hpwsc-clue-box"><strong>Hint:</strong> ${escapeHtml(entry.clue || "Unscramble the letters.")}<br><strong>Hint 2:</strong> ${escapeHtml(entry.clue2 || `A ${entry.answer.length}-letter word.`)}</div>
      ...
      <div class="hpwsc-actions">
        ...
        <button type="button" class="hpwsc-btn secondary" data-a="reveal-answers" ${isEnded()?"disabled":""}>Reveal Answers</button>
      </div>
    </div>`;
  }
  function renderWordList(){
    const solved=solvedSet(); const current=getCurrentEntry();
    return `<div class="hpwsc-panel hpwsc-word-panel"><h3>Words</h3><div class="hpwsc-word-list">${puzzleData.map(item=>{
      const isSolved=solved.has(item.answer); const active=current&&current.id===item.id&&!isSolved&&!isEnded(); const label=(state.revealed||isSolved)?item.answer:item.scrambled;
      return `<button type="button" class="hpwsc-word-item ${isSolved?"is-solved":""} ${active?"is-active":""}" data-word-id="${escapeHtml(item.id)}"><span>${escapeHtml(label)}</span>${isSolved?`<span class="hpwsc-word-check">✓</span>`:""}</button>`;
    }).join("")}</div></div>`;
  }
  ...
  function render(){
    ...
    mount.innerHTML=`<div class="hpwsc-shell"><div class="hpwsc-card">
      <div class="hpwsc-status"><span class="hpwsc-status-msg">${escapeHtml(statusMessage())}</span></div>
      <div class="hpwsc-layout">
        <div class="hpwsc-panel">
          ${puzzle.theme ? `<div style="text-align:center;"><span class="hpwsc-theme">${escapeHtml(puzzle.theme)}</span></div>` : ""}
          <div class="hpwsc-progress-wrap"><div class="hpwsc-progress-line"><span>Progress</span><span>${solvedCount().toLocaleString()} / ${puzzleData.length.toLocaleString()} words</span></div><div class="hpwsc-progress-bar"><div class="hpwsc-progress-fill" style="width:${progressPercent()}%"></div></div></div>
          ${renderCurrent()}
        </div>
        ${renderWordList()}
      </div>
  ...
  function flash(message){ const msg=mount?.querySelector(".hpwsc-status-msg"); if(!msg) return; const original=msg.textContent; msg.textContent=message; msg.style.color="#ED1B24"; setTimeout(()=>{ const current=mount?.querySelector(".hpwsc-status-msg"); if(!current) return; current.textContent=statusMessage()||original; current.style.color=""; },1200); }
  function emitStateChange(status){
    const detail={puzzleType:"word-scramble", puzzleId:puzzle?.puzzleId, storageKey:storageKey(), status};
  ...
  function updateAfterChange(status, fullRender=true){ writeState(); if(fullRender) render(); else { render(); } emitStateChange(status); }
  ...
  document.addEventListener("keydown", event=>{
    ...
    const active=document.activeElement; const typing=active && ["INPUT","TEXTAREA","SELECT"].includes(active.tagName);
    if(typing) return;
