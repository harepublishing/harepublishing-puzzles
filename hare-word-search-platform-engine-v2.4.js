/* =========================================================
   ...
   Updated: 2026-06-13 v2.4 — compact Word Scramble-sized Success/Reveal card aligned to Word Search

   Suggested filename:
   hare-word-search-platform-engine-v2.4.js
...
window.HareWordSearchEngine = {
  init({
    ...
    function trySelectPath(path) {
      ...
      const previousWordListScrollTop = wordListEl ? wordListEl.scrollTop : 0;

      saveState();
      render();

      const restoredWordListEl = mount.querySelector("#hp-ws-word-list");
      if (restoredWordListEl) restoredWordListEl.scrollTop = previousWordListScrollTop;
    ...
    function renderRecommendationHtml(){
      if (typeof window.HareWordSearchFindNextPuzzle === "function") {
        ...
        if (nextPuzzle && nextPuzzle.puzzleId) {
          return `<div class="hp-recommend-card">
            ...
            <div class="hp-recommend-copy">Your next available Word Search is ready. Keep the fun going with another word puzzle.</div>
            <button class="hp-link-btn primary" data-a="load-puzzle" data-puzzle-id="${escapeHtml(nextPuzzle.puzzleId)}">Word Search Puzzle #${escapeHtml(nextPuzzle.puzzleId)}</button>
          </div>`;
        }
      }
      return `<div class="hp-recommend-card"><div class="hp-recommend-title">All caught up!</div><div class="hp-recommend-copy">Congratulations — every available Word Search puzzle has been played.</div><span class="hp-link-btn secondary full" role="status">Congratulations!</span></div>`;
    }
    ...
    function renderOverlayContent() {
      ...
      if (state.solved) {
        ...
        overlayTextEl.innerHTML = `<div class="hp-modal-lead">Congratulations — you found every hidden word!</div><div class="hp-modal-subtext">Great job solving this Word Search puzzle.</div>${renderRecommendationHtml()}`;
        return;
      }
      ...
      if (state.revealed) {
        ...
        overlayTextEl.innerHTML = `<div class="hp-modal-lead">Completed with a little help —<br>the hidden words were revealed.</div>${renderRecommendationHtml()}`;
      }
    }
      ...
      mount.innerHTML = `
        ...
        <div class="hp-overlay" id="hp-ws-overlay" aria-hidden="true">
          <div class="hp-modal" role="dialog" aria-modal="true" aria-label="Word Search result">
            <span id="hp-ws-overlay-icon" class="material-symbols-outlined" aria-hidden="true">celebration</span>

            <h3 id="hp-ws-overlay-title">You Solved the Word Search!</h3>

            <div class="hp-result-meta">
              <div class="hp-result-puzzle-title" id="hp-ws-badge-id"></div>
              <div class="hp-result-stats-line" id="hp-ws-badge-meta"></div>
            </div>

            <div id="hp-ws-overlay-text">
            ...
            <div class="hp-modal-actions">
              ...
              <button class="hp-link-btn danger final-action" data-a="reset-puzzle">Start Over</button>
            </div>
