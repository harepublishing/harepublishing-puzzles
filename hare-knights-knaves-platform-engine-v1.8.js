...
window.HareKnightsKnavesPlatformEngine = {
  init({
    ...
    function scheduleMessageSounds(count) {
      const key = `${getPuzzleId()}_${currentMode}`;
      if (messageSoundPlayedKeys.has(key)) return;
      messageSoundPlayedKeys.add(key);
      for (let i = 0; i < count; i++) {
        window.setTimeout(() => playMedievalMessageTap(i === count - 1), i * 380 + 90);
      }
    }
      ...
      return `<div class="hp-knights-statements">
        <div class="hp-knights-message-list">
          ${statements.map((st, index) => {
            ...
            return `<div class="hp-knights-message hp-knights-message-${side}" style="animation-delay:${index * 380}ms;">
    ...
    function renderUI() {
      ...
      mount.innerHTML = `<style>
        ...
        #hp-knights-container .hp-knights-message{display:grid;grid-template-columns:62px auto;gap:9px;align-items:center;width:fit-content;max-width:min(72%,620px);opacity:0;transform:translateY(8px) scale(.985);animation:hpKnightsMessagePop .44s cubic-bezier(.2,.9,.2,1) forwards;}
      ...
      scheduleMessageSounds((cfg.statements || []).length);
