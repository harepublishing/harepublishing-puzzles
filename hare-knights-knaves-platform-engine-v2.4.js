  ...
  init({
    ...
    function getOverlayNextActionsHtml() {
      ...
      if (options?.preferredLevelExhausted && options?.alternatePuzzle) {
        ...
        return `<span class="hp-knights-caught-up">You're caught up on ${escapeHtml(preferred)}.</span><button type="button" class="hp-knights-secondary" data-a="close-overlay">Check Back Tomorrow</button><button type="button" class="hp-knights-primary" data-a="request-next" data-next-kind="alternate">Try ${escapeHtml(altMode)} #${escapeHtml(alt.puzzleId)}</button>`;
      }
      if (options?.allDone) {
