/* =========================================================
   HARE PUBLISHING SUDOKU PLATFORM ENGINE
   Version 4.7

   Platform version of the unified Sudoku engine.
   - Receives one Sudoku puzzle set from the platform page
...
window.HareSudokuPlatformEngine = {
  init({
    ...
    if (!container) {
      console.error("HareSudokuPlatformEngine: puzzle container missing.");
      return;
    }
    ...
    if (!container.classList.contains("hp-sudoku")) {
      console.warn("HareSudokuPlatformEngine: container is not marked hp-sudoku. Skipping mount.");
      return;
    }
    ...
    if (typeof container.__hpSudokuPlatformCleanup === "function") {
      try {
        ...
        console.warn("HareSudokuPlatformEngine: previous instance cleanup failed.", err);
      }
    }
    ...
    if (!mount) {
      console.error("HareSudokuPlatformEngine: .hp-mount element missing inside puzzle container.");
      return;
    }
    ...
    if (!pageData && dataEl) {
      try {
        ...
        console.error("HareSudokuPlatformEngine: puzzle data block contains invalid JSON.", err);
      }
    }
    ...
    const SUDOKU_MODE_ORDER = ["easy", "medium", "hard"];
    ...
    const MODE_META = {
      easy: {
        ...
        schemaDescription: "Play and solve an easy Sudoku puzzle by Hare Publishing. Includes notes, timer, checking, hints, reveal, and progress saving.",
        solvedTitle: "You Solved the Easy Sudoku Puzzle!",
        solvedStat: "Easy Sudoku Solved! 🎉 ",
        solvedText: "Congratulations — you did it! Try another difficulty in this Sudoku set, or explore more free online puzzles.",
        revealedText: "Here is the completed Easy Sudoku puzzle. Try another difficulty in this Sudoku set, or explore more free online puzzles.",
      ...
      medium: {
        ...
        schemaDescription: "Play and solve a medium Sudoku puzzle by Hare Publishing. Includes notes, timer, checking, hints, reveal, and progress saving.",
        solvedTitle: "You Solved the Medium Sudoku Puzzle!",
        solvedStat: "Medium Sudoku Solved! 🎉 ",
        solvedText: "Congratulations — you did it! Try another difficulty in this Sudoku set, or explore more free online puzzles.",
        revealedText: "Here is the completed Medium Sudoku puzzle. Try another difficulty in this Sudoku set, or explore more free online puzzles.",
      ...
      hard: {
        ...
        schemaDescription: "Play and solve a hard Sudoku puzzle by Hare Publishing. Includes notes, timer, checking, hints, reveal, and progress saving.",
        solvedTitle: "You Solved the Hard Sudoku Puzzle!",
        solvedStat: "Hard Sudoku Solved! 🎉 ",
        solvedText: "Congratulations — you did it! Try another difficulty in this Sudoku set, or explore more free online puzzles.",
        revealedText: "Here is the completed Hard Sudoku puzzle. Try another difficulty in this Sudoku set, or explore more free online puzzles.",
    ...
    function buildPuzzlesFromData(data) {
      ...
      const sharedPuzzleId = String(data.puzzleId || data.id || "").trim();
      const result = {};
      ...
      SUDOKU_MODE_ORDER.forEach(mode => {
        ...
        const puzzleId = String(item.puzzleId || item.id || sharedPuzzleId || "").trim();
    ...
    if (!availableModes.length) {
      showConfigError("At least one Sudoku puzzle is required. Add easy, medium, or hard puzzle data.");
      return;
    }
    ...
    const modesNeedingCompactMigration = new Set();
    ...
    function getCompletedCells(mode) {
      const cfg = PUZZLES[mode];
      return Array.from({ length: 81 }, (_, index) => ({
        value: cfg.puzzle[index] === "." ? cfg.solution[index] : "",
    ...
    function loadState(mode) {
      try {
        ...
        if (merged.solved || merged.revealed) {
          // Finished boards are reconstructed from the puzzle's bundled solution.
          // The solution never needs to remain in browser storage.
          merged.cells = getCompletedCells(mode);
          merged.history = [];
          if (Array.isArray(parsed?.cells)) modesNeedingCompactMigration.add(mode);
        } else {
    ...
    function getPersistableState(state) {
      ...
      if (cleanState.solved || cleanState.revealed) {
        // Completion records keep only the result metadata. The board can always
        // be rebuilt from the puzzle and solution already loaded by the page.
        delete cleanState.cells;
      } else {
    ...
    function saveState(mode, state, { refreshBackup = false } = {}) {
      try {
        const storageKey = getSaveKey(mode);
        const serialized = JSON.stringify(getPersistableState(state));
        localStorage.setItem(storageKey, serialized);

        // The platform core keeps the previous value as a recovery backup.
        // A second identical completion write replaces any old full-board backup
        // with the same compact record without changing completion status.
        if (refreshBackup) localStorage.setItem(storageKey, serialized);
      } catch (err) {
        console.warn("HareSudokuPlatformEngine: unable to save puzzle progress.", err);
      }
    }
    ...
    // Silently migrate older completed records. Do not emit a solve/reveal event:
    // migration must never add another completion to statistics or analytics.
    modesNeedingCompactMigration.forEach(mode => {
      saveState(mode, states[mode], { refreshBackup: true });
    });
    ...
    function hasPersistableProgress(state = getState()) {
      if (state.solved || state.revealed) return true;
      if (Number(state.elapsed || 0) > 0) return true;
      return Array.isArray(state.cells) && state.cells.some(cell =>
        String(cell?.value || "").trim() ||
        (Array.isArray(cell?.notes) && cell.notes.some(Boolean))
      );
    }
    ...
    function saveCompletedState(action) {
      saveState(currentMode, getState(), { refreshBackup: true });
      emitSudokuStateChange(currentMode, action);
    }
    ...
    function injectSchema() {
      ...
      const schemaDescription = puzzleId
        ? `Play Sudoku Puzzle #${puzzleId} online from Hare Publishing. Choose Easy, Medium, or Hard and save your progress as you solve.`
        : "Play Sudoku online from Hare Publishing. Choose Easy, Medium, or Hard and save your progress as you solve.";
      ...
      const schemaData = {
        ...
        "keywords": ["Sudoku", "number puzzle", "logic puzzle", "daily puzzle", "online puzzle", "Hare Publishing"],
        "audience": {
          "@type": "PeopleAudience",
          "suggestedMinAge": "8"
        },
    ...
    function getOverlayStats() {
      if (typeof window.HareSudokuGetStats === "function") {
        try {
          return window.HareSudokuGetStats();
        } catch (err) {
    ...
    function getOverlayModeSummaryHtml() {
      return `
        <div class="hp-overlay-mode-summary" aria-label="Sudoku set status">
    ...
    function renderOverlayNextPanel() {
      ...
      const cfg = getConfig();
      const remainingModes = getRemainingModeLabels();
      const hasRemainingModes = remainingModes.length > 0;
      let options = null;
      if (typeof window.HareSudokuGetNextPuzzleOptions === "function") {
        try { options = window.HareSudokuGetNextPuzzleOptions({ excludePuzzleId: cfg.puzzleId }); } catch {}
      }

      const preferred = options && options.preferredLabel ? options.preferredLabel : "your level";
      const preferredLower = String(preferred || "your level").toLowerCase();
      const nextPreferred = options && options.nextPreferred;
      const alternate = options && options.alternate;
      const anyUnfinished = options && options.anyUnfinished;
      const shareButton = `<button class="hp-sudoku-next-panel-btn secondary share-action" type="button" data-a="share">Share This Puzzle</button>`;

      if (alternate && !nextPreferred) {
        const altLabel = alternate.mode ? (alternate.mode.charAt(0).toUpperCase() + alternate.mode.slice(1)) : "Next";
        const remainingCopy = hasRemainingModes
          ? `${escapeHtml(joinList(remainingModes))} ${remainingModes.length === 1 ? "is" : "are"} still available in this puzzle set.`
          : "Another Sudoku challenge is still available.";
        overlayNextPanelEl.innerHTML = `
          <div class="hp-sudoku-overlay-next-copy">
            <strong>You're caught up on ${escapeHtml(preferred)}!</strong>
            <span>Check back tomorrow for the next ${escapeHtml(preferredLower)} puzzle.</span>
            <span>${remainingCopy}</span>
          </div>
          <div class="hp-sudoku-next-panel-actions">
            <button class="hp-sudoku-next-panel-btn primary" type="button" data-a="request-next-sudoku" data-next-kind="alternate">Try ${escapeHtml(altLabel)} #${escapeHtml(alternate.puzzleId)}</button>
            ${shareButton}
          </div>
        `;
      ...
      if (nextPreferred) {
        const isProgress = nextPreferred.status === "in-progress";
        const label = nextPreferred.mode ? `${nextPreferred.mode.charAt(0).toUpperCase() + nextPreferred.mode.slice(1)} Sudoku #${nextPreferred.puzzleId}` : `Sudoku #${nextPreferred.puzzleId}`;
        const messageTitle = isProgress ? "Pick up where you left off." : "Try another Sudoku puzzle.";
        const messageCopy = isProgress
          ? "Keep solving and building your Sudoku streak."
          : "Keep your Sudoku streak going with the next available puzzle.";
        const remainingCopy = hasRemainingModes
          ? `${escapeHtml(joinList(remainingModes))} ${remainingModes.length === 1 ? "is" : "are"} still available in this puzzle set.`
          : "";
        overlayNextPanelEl.innerHTML = `
          <div class="hp-sudoku-overlay-next-copy">
            ...
            ${remainingCopy ? `<span>${remainingCopy}</span>` : ""}
          </div>
          <div class="hp-sudoku-next-panel-actions">
            <button class="hp-sudoku-next-panel-btn primary" type="button" data-a="request-next-sudoku" data-next-kind="recommended">${isProgress ? "Continue " : "Play "}${escapeHtml(label)}</button>
            ${shareButton}
          </div>
        `;
        overlayNextPanelEl.hidden = false;
    ...
    function renderOverlayContent() {
      ...
      if (!badgeIdEl || !badgeTimeEl || !overlayIconEl || !overlayStatusEl || !overlayTitleEl || !overlayTextEl || !overlayFooterEl) return;
      ...
      overlayStatusEl.textContent = state.solved ? "Solved" : state.revealed ? "Revealed" : "Complete";
      overlayTextEl.innerHTML = `
        <button class="hp-sudoku-view-finished" type="button" data-a="view-finished-puzzle">
          ${state.solved ? "View Completed Puzzle" : "View Solution"}
        </button>
      `;
      overlayFooterEl.textContent = "";
...
<style>
  ...
  #hp-sudoku-container.hp-sudoku *,
  #hp-sudoku-container.hp-sudoku *::before,
  #hp-sudoku-container.hp-sudoku *::after {
    box-sizing: border-box;
  }

  #hp-sudoku-container.hp-sudoku:focus,
  #hp-sudoku-container.hp-sudoku:focus-visible {
    outline: none;
  }
  ...
  #hp-sudoku-container.hp-sudoku .hp-col-right {
    flex: 1;
    min-width: 300px;
    max-width: 400px;
  }

  #hp-sudoku-container.hp-sudoku .hp-grid {
    display: grid;
  ...
  #hp-sudoku-container.hp-sudoku .hp-cell {
    background: #fff;
    position: relative;
    display: block;
    cursor: pointer;
  ...
  #hp-sudoku-container.hp-sudoku .hp-cell::before {
    content: "";
    display: block;
    padding-bottom: 100%;
  }

  #hp-sudoku-container.hp-sudoku .hp-cell-inner {
    position: absolute;
    inset: 0;
    display: flex;
    align-items: center;
  ...
  #hp-sudoku-container.hp-sudoku .hp-value {
    font-size: 24px;
    line-height: 1;
    user-select: none;
  }

  #hp-sudoku-container.hp-sudoku .hp-cell.selected {
    background-color: #fff;
    box-shadow: inset 0 0 0 4px #F7941C;
    z-index: 10;
  }

  #hp-sudoku-container.hp-sudoku .hp-cell:focus-visible {
    box-shadow: inset 0 0 0 4px #F7941C;
  }

  #hp-sudoku-container.hp-sudoku .hp-cell.user-input .hp-value {
    color: #107FBB;
    font-weight: 800;
  }

  #hp-sudoku-container.hp-sudoku .hp-cell.bL {
    border-left: 2px solid #000;
  }

  #hp-sudoku-container.hp-sudoku .hp-cell.bR {
    border-right: 2px solid #000;
  }

  #hp-sudoku-container.hp-sudoku .hp-cell.bT {
    border-top: 2px solid #000;
  }

  #hp-sudoku-container.hp-sudoku .hp-cell.bB {
  ...
  #hp-sudoku-container.hp-sudoku .hp-cell.has-value .hp-note-box {
    display: none;
  }

  #hp-sudoku-container.hp-sudoku .hp-btn {
  ...
  #hp-sudoku-container.hp-sudoku .hp-btn-sm {
    border: 1px solid #ddd;
    background: #fff;
    color: #333;
    min-height: 42px;
  ...
  #hp-sudoku-container.hp-sudoku .hp-btn-sm.active {
    border-color: #00A54F;
    color: #00A54F;
    box-shadow: inset 0 0 0 2px #00A54F;
  }

  #hp-sudoku-container.hp-sudoku .hp-btn-sm.reveal {
    color: #107FBB;
    border-color: rgba(16,127,187,.4);
  }

  #hp-sudoku-container.hp-sudoku .hp-btn-sm[data-a="reset-board"] {
    color: #ED1B24;
    border-color: rgba(237,27,36,.4);
  }

  #hp-sudoku-container.hp-sudoku .hp-action-grid .hp-btn-sm:hover,
  #hp-sudoku-container.hp-sudoku .hp-action-grid .hp-btn-sm:focus-visible {
    transform: translateY(-2px);
    box-shadow: 0 10px 22px rgba(0,0,0,.10);
    outline: none;
  }

  #hp-sudoku-container.hp-sudoku .hp-action-grid .hp-btn-sm[data-a="reset-board"]:hover,
  #hp-sudoku-container.hp-sudoku .hp-action-grid .hp-btn-sm[data-a="reset-board"]:focus-visible {
    background: #ED1B24;
    border-color: #ED1B24;
    color: #fff;
  }

  #hp-sudoku-container.hp-sudoku .hp-action-grid .hp-btn-sm.reveal:hover,
  #hp-sudoku-container.hp-sudoku .hp-action-grid .hp-btn-sm.reveal:focus-visible {
    background: #107FBB;
    border-color: #107FBB;
    color: #fff;
  }
  ...
  #hp-sudoku-container.hp-sudoku .hp-modal {
    width: 100%;
    max-width: 520px;
    background: #fff;
    border-radius: 14px;
  ...
  #hp-sudoku-container.hp-sudoku .hp-modal h3 {
    margin: 6px 0 8px;
    font-size: 22px;
    line-height: 1.2;
    font-weight: 900;
  ...
  #hp-sudoku-container.hp-sudoku .hp-modal p,
  #hp-sudoku-container.hp-sudoku .hp-modal-subtext {
    margin: 0 0 8px;
    font-size: 14px;
    color: #555;
    line-height: 1.35;
  ...
  #hp-sudoku-container.hp-sudoku .hp-modal-actions {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 10px;
  }

  #hp-sudoku-container.hp-sudoku .hp-link-btn {
    display: inline-flex;
    align-items: center;
    justify-content: center;
    min-height: 42px;
  ...
  #hp-sudoku-container.hp-sudoku .hp-link-btn.primary {
    border-color: #00A54F;
    color: #00A54F;
  }

  #hp-sudoku-container.hp-sudoku .hp-link-btn.secondary {
    border-color: #107FBB;
    color: #107FBB;
  }

  #hp-sudoku-container.hp-sudoku .hp-link-btn.neutral {
    border-color: #ddd;
    color: #333;
  }

  #hp-sudoku-container.hp-sudoku .hp-link-btn.danger {
    border-color: #ED1B24;
    color: #ED1B24;
  }

  #hp-sudoku-container.hp-sudoku .hp-link-btn.full {
    grid-column: 1 / -1;
  }
  ...
  #hp-sudoku-container.hp-sudoku .hp-overlay-status {
    margin: 0 0 8px !important;
  ...
  #hp-sudoku-container.hp-sudoku .hp-sudoku-view-finished {
  ...
  #hp-sudoku-container.hp-sudoku .hp-sudoku-view-finished:hover,
  #hp-sudoku-container.hp-sudoku .hp-sudoku-view-finished:focus-visible {
    color: #680099 !important;
    outline: 2px solid rgba(104,0,153,.25) !important;
    outline-offset: 2px !important;
    border-radius: 6px !important;
  }


  #hp-sudoku-container.hp-sudoku #hp-overlay .hp-overlay-close {
    position: absolute !important;
    top: 14px !important;
  ...
  #hp-sudoku-container.hp-sudoku #hp-overlay .hp-overlay-close:hover,
  #hp-sudoku-container.hp-sudoku #hp-overlay .hp-overlay-close:focus-visible {
    background: #680099 !important;
    color: #fff !important;
  }
  #hp-sudoku-container.hp-sudoku #hp-overlay .hp-overlay-close .material-symbols-outlined {
    font-size: 28px !important;
    line-height: 1 !important;
  }
  ...
  /* =========================================================
     SUDOKU SUCCESS / REVEAL CARD - v4.1
     Match Knights & Knaves compact overlay while preserving
     Sudoku theme color and Share This Puzzle functionality.
     ========================================================= */
  #hp-sudoku-container.hp-sudoku #hp-overlay .hp-modal {
  ...
  #hp-sudoku-container.hp-sudoku .hp-sudoku-overlay-message {
  ...
  #hp-sudoku-container.hp-sudoku .hp-sudoku-overlay-next-copy {
    margin: 18px auto 16px !important;
    display: flex !important;
    flex-direction: column !important;
    align-items: center !important;
  ...
  #hp-sudoku-container.hp-sudoku .hp-sudoku-overlay-next-copy strong {
    font-size: 22px !important;
    color: #555 !important;
    font-weight: 900 !important;
  }

  #hp-sudoku-container.hp-sudoku .hp-sudoku-overlay-next-copy span {
    display: block !important;
  }
  ...
  @media (max-width: 640px) {
     ...
     <div class="hp-puzzle-date">
        ...
        <div class="hp-layout">
          ...
          <div class="hp-col-right hp-rs-right">
            ...
            <div class="hp-action-grid">
              <button class="hp-btn-sm" data-a="reset-board" aria-label="Start over">Start Over</button>
              <button class="hp-btn-sm reveal" data-a="reveal-answer" aria-label="Reveal answers">Reveal Answers</button>
            </div>
        ...
        <div class="hp-overlay" id="hp-overlay" aria-hidden="true">
          <div class="hp-modal" role="dialog" aria-modal="true" aria-label="Puzzle complete">
            <button class="hp-overlay-close" type="button" data-a="close-solved" aria-label="Close success card"><span class="material-symbols-outlined" aria-hidden="true">close</span></button>
            <div id="hp-overlay-icon" class="material-symbols-outlined">celebration</div>
            <div id="hp-overlay-status" class="hp-overlay-status">Solved</div>
            ...
            <div class="hp-overlay-stats" id="hp-overlay-stats" aria-label="Sudoku statistics"></div>
      ...
      overlayStatusEl = mount.querySelector("#hp-overlay-status");
    ...
    function resetCurrentPuzzle() {
      ...
      const prompt = wasFinished
        ? "Start over? This will erase this level's completed result and make it a new puzzle again."
        : "Clear Board?";
      if (!confirm(prompt)) return;

      pauseTimer(false);
      states[currentMode] = defaultState();
      const storageKey = getSaveKey(currentMode);

      // Start Over is an intentional downgrade from finished/in-progress to new.
      // Remove both the active record and the protected recovery copy so the
      // level behaves exactly as if it had never been opened.
      try {
        if (Core && typeof Core.removeSavedState === "function") {
          Core.removeSavedState(storageKey, { allowDowngrade: true });
        } else {
          window.HarePuzzleProgressCore?.allowDowngradeOnce?.(storageKey);
          localStorage.removeItem(storageKey);
        }
        localStorage.removeItem(`hp_backup__${storageKey}`);
      } catch (err) {
        console.warn("HareSudokuPlatformEngine: unable to erase puzzle progress.", err);
      }
      ...
      statEl.textContent = "Tap a cell to begin";
      updateCompletionStatus();
      emitSudokuStateChange(currentMode, "reset");
    }
      ...
      mountClickHandler = (e) => {
        ...
        if (a === "request-next-sudoku") {
          ...
          if (typeof window.HareSudokuRequestNextSudoku === "function") {
            window.HareSudokuRequestNextSudoku({ source: "overlay", kind: aBtn.dataset.nextKind || "recommended" });
          }
          return;
        }
    ...
    beforeUnloadHandler = () => {
      ...
      // Do not recreate an empty record after Start Over. A save begins only
      // after the player has actually interacted with this level again.
      // Save directly without broadcasting a state-change event. Dispatching
      // while the browser is hiding/freezing the page causes other components
      // (notably the Sudoku carousel) to rebuild their DOM during BFCache entry,
      // which can leave newly-created lazy images blank after Back navigation.
      if (hasPersistableProgress(state)) saveState(currentMode, state);
    };
...
window.HareSudokuEngine = window.HareSudokuPlatformEngine;
