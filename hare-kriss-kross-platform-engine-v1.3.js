...
`;
    ...
    function adjustBoardSize() {
      ...
      const wrapEl = mount.querySelector(".hp-kk-board-wrap");
      if (!boardEl || !wrapEl || !rowCount || !colCount) return;
      ...
      const gap = window.matchMedia("(max-width: 700px)").matches ? 2 : 3;
      const availableWidth = Math.max(0, wrapEl.clientWidth - padX - 2);
      const availableHeight = Math.max(0, wrapEl.clientHeight - padY - 2);
      const byWidth = Math.floor((availableWidth - ((colCount - 1) * gap)) / colCount);
      const byHeight = availableHeight ? Math.floor((availableHeight - ((rowCount - 1) * gap)) / rowCount) : byWidth;
      const maxCell = window.matchMedia("(max-width: 700px)").matches ? 27 : 34;
      const minCell = window.matchMedia("(max-width: 560px)").matches ? 14 : 18;
      const cellSize = Math.max(minCell, Math.min(maxCell, byWidth, byHeight || byWidth));
