...
(function(){
  ...
  const ENGINE_FILE = "hare-wordrow-platform-engine-v1.1.js";

  function detectBase(){
    const scripts = Array.from(document.scripts || []);
    const s = scripts.find(x => (x.src || "").includes("hare-wordrow-platform-engine-v1.1.js")) ||
  ...
  function renderShell(){
    root.innerHTML = `
      <div class="hpwr-header">
        <p class="hpwr-kicker">Free Online Wordrow Puzzle</p>
        <h1 class="hpwr-main-title">Daily Wordrow</h1>
        <p class="hpwr-main-subtitle">Guess the hidden five-letter word. New puzzles are added by date and past puzzles stay available in the archive.</p>
      </div>
      <div class="hpwr-play-panel" id="hpwr-play-panel">
        <div class="hpwr-layout">
          <main class="hpwr-game-card" id="hpwr-game-card"><div class="hpwr-loading">Loading Wordrow puzzle...</div></main>
          <aside class="hpwr-side" id="hpwr-side" aria-label="Wordrow tools"></aside>
        </div>
      </div>
  ...
  function renderGame(){
    ...
    puzzleArea.innerHTML = `
      ...
      <div class="hpwr-help"><strong>How to play:</strong> Enter a five-letter word. Green letters are correct, orange letters are in the word but in the wrong spot, and gray letters are not in the word.</div>
  ...
  async function renderSide(){
    if (!sideArea) return;
    const [stats, next, future] = await Promise.all([getStats(), nextForUser(currentPuzzle && currentPuzzle.puzzleId), nextFuture()]);
    const nextHtml = next ? `<div class="hpwr-next-box"><div class="hpwr-next-label">Next For You</div><div class="hpwr-next-title">Wordrow #${esc(next.puzzleId)}</div><div class="hpwr-next-date">${esc(shortDate(next.puzzleDate))}</div><button type="button" class="hpwr-side-btn primary" data-hpwr-load="${esc(next.puzzleId)}"><span class="material-symbols-outlined">arrow_forward</span>Play Your Next Puzzle</button></div>` : `<div class="hpwr-next-box"><div class="hpwr-next-title">All caught up!</div><div class="hpwr-next-date">Check the archive or come back for the next puzzle.</div></div>`;
    const timerHtml = future ? `<div class="hpwr-countdown" data-hpwr-next-date="${esc(future.puzzleDate)}"><div class="hpwr-countdown-label">Next Wordrow</div><div class="hpwr-countdown-time" id="hpwr-countdown-time">${esc(shortDate(future.puzzleDate))}</div></div>` : "";
    sideArea.innerHTML = `
      <div class="hpwr-side-card"><h3 class="hpwr-side-title"><span class="material-symbols-outlined">help</span>Help</h3><p class="hpwr-side-text">Guess the hidden five-letter word in six tries. Use the tile colours to narrow down the answer.</p></div>
      <div class="hpwr-side-card">${nextHtml}${timerHtml}</div>
      <div class="hpwr-side-card"><h3 class="hpwr-side-title"><span class="material-symbols-outlined">explore</span>Explore</h3><a class="hpwr-side-btn blue" href="${ARCHIVE_URL}"><span class="material-symbols-outlined">folder_open</span>Wordrow Archive</a><a class="hpwr-side-btn purple" href="${MORE_PUZZLES_URL}"><span class="material-symbols-outlined">apps</span>More Puzzles</a></div>
      <div class="hpwr-side-card full"><h3 class="hpwr-side-title"><span class="material-symbols-outlined">bar_chart</span>Stats</h3><div class="hpwr-stats"><div class="hpwr-stat"><strong>${num(stats.streak)}</strong><span>Day Streak</span></div><div class="hpwr-stat"><strong>${num(stats.solved)}</strong><span>Solved</span></div><div class="hpwr-stat"><strong>${num(stats.progress)}</strong><span>In Progress</span></div><div class="hpwr-stat"><strong>${num(stats.played)}</strong><span>Played</span></div></div><div class="hpwr-support-links"><a href="${FEEDBACK_URL}">Feedback</a><a href="${BUG_URL}">Report Bug</a></div></div>`;
    updateCountdown();
  }
  function updateCountdown(){
    const box = document.querySelector("[data-hpwr-next-date]");
    const out = document.getElementById("hpwr-countdown-time");
    if (!box || !out) return;
    const d = parseDate(box.dataset.hpwrNextDate); if (!d) return;
    const ms = d.getTime() - Date.now();
    if (ms <= 0) { out.textContent = "Available soon"; return; }
    const days = Math.floor(ms/86400000), hrs = Math.floor((ms%86400000)/3600000), mins = Math.floor((ms%3600000)/60000);
    out.textContent = days > 0 ? `${days}d ${hrs}h` : `${hrs}h ${mins}m`;
  }
  ...
  function showModal(type){
    ...
    const icon = solved ? "trophy" : lost ? "sports_score" : "visibility";
    const text = solved ? `You solved ${currentPuzzle.title} in ${currentState.guesses.length}/${maxGuesses}.` : `The word was ${currentPuzzle.answer}.`;
    document.body.insertAdjacentHTML("beforeend", `<div class="hpwr-overlay on" id="hpwr-overlay"><div class="hpwr-modal" role="dialog" aria-modal="true"><div class="hpwr-modal-icon"><span class="material-symbols-outlined">${icon}</span></div><h3>${esc(title)}</h3><p>${esc(text)}</p><div class="hpwr-badges"><span class="hpwr-badge">${esc(currentPuzzle.title)}</span><span class="hpwr-badge">${esc(prettyDate(currentPuzzle.puzzleDate))}</span></div><div class="hpwr-modal-actions"><button type="button" class="hpwr-link-btn primary full" data-hpwr-action="next"><span class="material-symbols-outlined">arrow_forward</span>Play Your Next Puzzle</button><a class="hpwr-link-btn blue" href="${MORE_PUZZLES_URL}">More Online Puzzles</a><a class="hpwr-link-btn" href="${SHOP_URL}">Get Puzzle Books</a><button type="button" class="hpwr-link-btn" data-hpwr-action="close-modal">Back to Puzzle</button></div></div></div>`);
  }
  ...
  document.addEventListener("click", e => {
    ...
    if (a === "reset") reset(); else if (a === "reveal") reveal(); else if (a === "next") playNext(); else if (a === "share") share(); else if (a === "close-modal") document.getElementById("hpwr-overlay")?.remove();
  });
  ...
  window.HareWordrowPlatform = { init, loadPuzzle, playNext, loadIndex, loadYear, availableIndex, getStats, getStatus, readState, stateKey, nextForUser, config:{BASE,DATA_BASE,ICON_URL,WORDROW_URL,ARCHIVE_URL,STORAGE_PREFIX,PRIMARY} };
