  ...
  init({
    ...
    function getOverlayNextActionsHtml(remainingText) {
      if (typeof window.HareKnightsGetNextPuzzleOptions !== "function") {
        return typeof window.HareKnightsRequestNextPuzzle === "function"
          ? `<div class="hp-knights-modal-buttons"><button type="button" class="hp-knights-primary" data-a="request-next">Play Your Next Puzzle</button></div>`
          : `<div class="hp-knights-modal-buttons"><a class="hp-knights-primary" href="/puzzlers-hub">Puzzlers Hub</a></div>`;
      }
      ...
      if (actions.length) {
        return `${remainingText ? `<p class="hp-knights-overlay-message">${escapeHtml(remainingText)}</p>` : ""}<div class="hp-knights-modal-buttons">${actions.join("")}</div>`;
      }
      ...
      if (options?.preferredLevelExhausted && options?.alternatePuzzle) {
        ...
        return `<div class="hp-knights-overlay-next-copy"><strong>You're caught up on ${escapeHtml(preferred)}!</strong><span>Check back tomorrow for the next ${escapeHtml(preferred)} puzzle.</span>${remainingText ? `<span>${escapeHtml(remainingText)}</span>` : ""}</div><div class="hp-knights-modal-buttons"><button type="button" class="hp-knights-primary" data-a="request-next" data-next-kind="alternate">Try ${escapeHtml(altMode)} #${escapeHtml(alt.puzzleId)}</button></div>`;
      }
      if (options?.allDone) {
        return `<div class="hp-knights-overlay-next-copy"><strong>You're all caught up!</strong><span>Explore more puzzles in the Puzzlers Hub.</span></div><div class="hp-knights-modal-buttons"><a class="hp-knights-primary" href="/puzzlers-hub">Explore More Puzzles</a></div>`;
      }
      return `<div class="hp-knights-overlay-next-copy"><strong>You're all caught up!</strong><span>Check back tomorrow.</span></div>`;
    }
    ...
    function renderUI({ playMessages = allowNextMessageSound } = {}) {
      ...
      mount.innerHTML = `<style>
        ...
        #hp-knights-container .hp-knights-modal{background:#fff;width:min(500px,100%);border-radius:18px;padding:18px 20px;border:1px solid #e9eef3;box-shadow:0 18px 48px rgba(0,0,0,.18);text-align:center;color:#222;max-height:calc(100% - 24px);overflow-y:auto;position:relative;}
        ...
        #hp-knights-container .hp-knights-modal-actions{margin-top:10px;text-align:center;}
        #hp-knights-container .hp-knights-overlay-message{margin:14px auto 12px;font-size:20px;line-height:1.25;color:#222;font-weight:500;}
        #hp-knights-container .hp-knights-overlay-next-copy{margin:14px auto 12px;display:flex;flex-direction:column;align-items:center;gap:4px;color:#555;font-size:16px;line-height:1.25;font-weight:800;text-align:center;}
        #hp-knights-container .hp-knights-overlay-next-copy strong{font-size:18px;color:#555;font-weight:900;}
        #hp-knights-container .hp-knights-overlay-next-copy span{display:block;}
        #hp-knights-container .hp-knights-modal-buttons{display:flex;justify-content:center;align-items:center;gap:10px;flex-wrap:wrap;margin-top:10px;}
        #hp-knights-container .hp-knights-modal-actions button,#hp-knights-container .hp-knights-modal-actions a{border:2px solid transparent;border-radius:12px;min-height:36px;min-width:190px;padding:8px 18px;font-weight:900;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;justify-content:center;font-family:inherit;font-size:12px;line-height:1.1;}
